...
empty; old 20260807 candidate `INVALID-TIMELINE` unusable). That old candidate pair is now consumed by the external sealed blocked evidence `EV-E3-PHYS1API26-20260808-0001` (record_status=collected / overall=blocked / verdict=blocked, execution_mode=live, is_evidence=true; campaign-seal SHA-256 `ec55603a2555b967c5912e79a846262d262e53069e3f1275105af5ccf4ef245f` sealed_at 2026-08-08T09:53:23+08:00; hash-manifest SHA-256 `36852f6c6d9d6e27c6b5ee07bd6b6037e8aa2a22f1fdbf45591addb047f5f843`; legacy-pair-consumption-audit `id-consumption-audit-1.txt` SHA-256 `b530c438769e65390ee9065f918d1ea550a8f9745c61516b6899ff90c736e9c7`, 2026-08-10T10:43:09+08:00) and must never be reused. **Historical (2026-08-10 · `AUTH-E3-PHYS1API26-20260810-0002`)**: the user explicitly authorized a NEW full-whitelist campaign (see [`e3-physical-preflight-authorization-2026-08-10-0002.md`](../../docs/evidence/e3-physical-preflight-authorization-2026-08-10-0002.md); supersedes the consumed `AUTH-E3-PHYS1API26-20260810-0001`) with the new candidate pair `E3-PHYS-PREFLIGHT-20260810-0001` / `EV-E3-PHYS1API26-20260810-0001`, `attempt=initial`/retry N/A, any gate failure stops with no retry and no ID switch, two new-pair consumption audits (audit-1 before any `-TargetBindingConfirm`, audit-2 after the final ready freeze; both hash recorded), and a single memory-only `hdc list targets` host-prep exception (exactly one token, process-scope `PHYS_1_TARGET`, no output/persistence, runner HDC whitelist NOT expanded). Live requires the machine fresh confirmation (`-TargetBindingConfirm`) and the review record bindings; user readiness attestation does not replace them. This registration banned HDC (except the single memory-only host-prep `hdc list targets`). **Preflight complete (2026-08-30):** `AUTH-E3-PHYS1API26-20260829-0001` (pair `E3-PHYS-PREFLIGHT-20260829-0001` / `EV-E3-PHYS1API26-20260829-0001`, frozen tuple `PLA-AL10 7.0.0.102(SP8C00E102R7P3)`) executed all 13 gates, and its Live (evidence [`EV-E3-PHYS1API26-20260829-0001`](../../docs/evidence/e3-physical-preflight-api26-20260829-0001.md), `reviewed-pass/pass`, measured window 2026-08-30 10:14-10:31 +08:00) passed **S1-S7 all machine-verified — the project's first complete pass**: fd ownership (post-create fd=33 open), strict process-boundary terminals, deny full-window without B create, Settings app-info force-stop revoke, the frozen conflict code `2203002` verbatim, and final-stop cleanup. The S7 precondition calibration was verified as genuinely triggered (B's lingering `:vpn` process recorded `observed_only=true`, A gated normally — the same state had blocked the 20260828-0001 pair). Cleanup verified-clean; integrity empty; 166 whitelisted HDC processes. The AUTH is `consumed-pass`, the pair is consumed, and there is **no successor AUTH** — the E3-PHYS-PREFLIGHT preflight is complete. Scope: frozen-tuple E3 reachability only; not E4-E7, product, data-plane, or E8 conclusions; **E8 remains `CLOSED`** (opening requires an independent aggregate review plus the remaining gates, including the current-baseline E1). Historical pairs (20260828-0001 consumed-blocked, 20260825-0001 tuple-drift, 20260817-0002 order-invalid, 20260817-0001 operation-invalid, and earlier) must never be reused. Outside gate 4's exact host-prep and the runner's 22 allowlisted operations, all device commands, device process lists, and extra discovery remain forbidden. Host HDC0 uses only absolute `/usr/bin/ps -eo comm=,args=` output and compares only its first column. Any new physical campaign requires new user governance and a new AUTH.
