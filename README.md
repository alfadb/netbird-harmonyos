...
> **E3-PHYS-PREFLIGHT 预检已完结（2026-08-30）**：[`AUTH-E3-PHYS1API26-20260829-0001`](docs/evidence/e3-physical-preflight-authorization-2026-08-29-0001.md)（pair `E3-PHYS-PREFLIGHT-20260829-0001` / `EV-E3-PHYS1API26-20260829-0001`）完整执行 13 门，其 Live（[证据](docs/evidence/e3-physical-preflight-api26-20260829-0001.md) `reviewed-pass/pass`，Opus 0B/0M）**S1-S7 全部 machine-verified pass——项目首次完整 pass**：fd 所有权、进程边界、deny 路径、Settings 强行停止撤销、冲突码 `2203002`、收尾清理全部机器验证；S7 前置校准实证生效（B 的 `:vpn` 残留进程 observed-only 记录不门控、A 正常门控）；cleanup verified-clean。AUTH 已 `consumed-pass`、pair 已消费、**无后继 AUTH**。范围：仅覆盖冻结元组（HarmonyOS / PLA-AL10 / `PLA-AL10 7.0.0.102(SP8C00E102R7P3)` / API 26 / aarch64 / arm64-v8a）的 E3 可达性，**不等于 E4-E7/产品/数据面结论；E8 保持 `CLOSED`**（开放需独立聚合审查与其余门，含当前基线 E1）。历史 pair（20260828-0001 consumed-blocked、20260825-0001 tuple-drift、20260817-0002 order-invalid、20260817-0001 operation-invalid）均不得复用；runner 22 项白名单与 HDC 禁令沿历史登记。后续路线由用户另行治理。本文其余旧“当前/下一步”叙述均按历史快照理解。

**当前活跃开发工具链（2026-09-05 起）**：已迁移为 CLT `26.0.0.821` / HarmonyOS SDK API 26 **Release**（旧 `6.1.1.290` / `26.0.0.461` 双链路已退役），唯一版本基线见 [`docs/toolchain-baseline.md`](docs/toolchain-baseline.md)；N1BDISC 冻结判据的 SDK 锚点对账（9/9 PASS）见 [`docs/n1bdisc-sdk-anchor-reconciliation-20260905.md`](docs/n1bdisc-sdk-anchor-reconciliation-20260905.md)。
...
- [E3-PHYS-PREFLIGHT 2026-08-16 授权登记（0003，governance-review-blocked-retired）](docs/evidence/e3-physical-preflight-authorization-2026-08-16-0003.md)（[0002（governance-order-invalid-retired；未 Live、未 consumed）](docs/evidence/e3-physical-preflight-authorization-2026-08-16-0002.md)，[0001（gate-3 review-blocked retired）](docs/evidence/e3-physical-preflight-authorization-2026-08-16-0001.md)）
...
- [工具链版本基线（当前活跃开发）](docs/toolchain-baseline.md)：CLT 26.0.0.821 / API 26 Release
- [N1BDISC SDK 锚点对账（2026-09-05，9/9 PASS）](docs/n1bdisc-sdk-anchor-reconciliation-20260905.md)
