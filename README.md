...
E8 前只允许一个 `E3-PHYS-PREFLIGHT` campaign：在已冻结的一台具名 HarmonyOS `PLA-AL10` 设备上，用普通开发签名的纯 ArkTS/C 公共 VPN Extension A/B 探针判断 E3 可达性。隔离目录 `spikes/e3-vpn-extension-physical-preflight-hap/` 已完成 API 23 受限适配和本地 unsigned 构建，历史 `spikes/e3-vpn-extension-hap` 与 raw evidence 未修改；该快照不是已签名或 campaign 冻结制品。唯一一次最小只读发现的六条白名单设备 `shell` 已成功，冻结为 distribution `HarmonyOS`、完整 build `PLA-AL10 6.1.0.117(SP6C00E115R7P7)`、API `23`、kernel arch `aarch64` 与 app ABI `arm64-v8a`；真实 HDC endpoint/target 仅在仓外受控映射为 `PHYS-1`。唯一 signing enrollment 命令已获授权，但截至 `2026-08-05` 尚未执行；仅当 DevEco Studio 未自动安全采集 UDID 时，Windows 授权人员才可为普通开发 profile 输入执行一次 `bm get --udid`。这是已批准普通开发 profile 输入的最小实施边界，不扩展 campaign 或 evidence，也不是新的动态调整记录。除此之外，没有其他设备 `shell`、`install`、`send`、`start`、`stop` 或 campaign。签名/profile、已签名 A/B HAP、冻结源码/SDK/final hash、清理、采集/审查和 campaign 输入仍缺，计划保持 `blocked`，也未分配 evidence ID 或 verdict，E8 必须保持 `CLOSED`。完整受控流程见 [E3 预检计划](docs/e3-physical-preflight.md)与 [Windows + DevEco Studio 开发交接](docs/windows-development-handoff.md)。只有后续预检证据达到 `reviewed-pass/pass` 才满足 E8 `OPEN` 的必要条件，但仍须当前R0正式基线（现v0.74.7）的 E1 通过、全部目标元组/哈希一致和独立聚合审查，不能自动开放。E8 `OPEN` 只许可后续物理投入；E4-E7 完整 VPN/数据面义务仍须在具名物理设备 R2/R3 门完成。
