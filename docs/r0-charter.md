...
- `E3-PHYS-PREFLIGHT` 的历史复合输入冻结已完成，故 R0 checklist 对应历史输入项仍为 `[x]`；API23 initial live 因 build drift 为 `reviewed-pass/blocked`；API26 live 为 `consumed-blocked`。rebind `EV-E3-PHYS1REBIND7-20260806-0001` 已完成；`ADJ-20260806-0003` 冻结新元组；host reverify PASS；`ADJ-20260806-0004` build 确认 PASS（仅 build-confirm）；`ADJ-20260807-0002` 批准中文完整重跑，当前 `plan_status: pending-commit-freeze`（需 commit/freeze/dryrun/device-ready；禁 HDC）。R0 的渠道及其他完整支持维度仍未锁定，不能从输入冻结或证据审查通过推导 R0 退出。
...
- [x] 按 `ADJ-20260806-0003` 冻结 HarmonyOS 7 新元组并分配准备用新 campaign/evidence ID；host reverify PASS（HAP/signature/profile/member hashes 未变；不主张安装兼容性）；设备执行须再次确认；后续 API26 live `consumed-blocked` 与中文完整重跑见 `ADJ-20260807-0002`；当前 `plan_status: pending-commit-freeze`。
...
- [x] 锁定普通开发签名/profile、FINAL signed A/B HAP、冻结源码/SDK/final hash、runner（API26 blocked campaign 绑定 `19fc1a76e49b9dca66a8a0352cc6bc8291f2888e66b3ad72cdc8a91ed97312e7`；中文可用性修正待 commit 后重算）、selftest（历史 `f7156314031eaf3649dfc6e5e245b9fbba75944724ed03fa62b87a465188b39d`）、清理基线、采集/审查准备、角色、campaign ID、60 秒窗口和 Settings 预测路径；历史输入冻结事实不因后续 `plan_status` 变化回退；host reverify 已完成；新 campaign 须新 commit/freeze。
- [ ] 仅按 `E3-PHYS-PREFLIGHT` 在该具名设备执行一个 campaign（当前准备：`E3-PHYS-PREFLIGHT-20260807-0002` / `EV-E3-PHYS1API26-20260807-0002`，完整 1–7，prior blocked 绑定 `EV-E3-PHYS1API26-20260807-0001`；历史 `E3-PHYS-PREFLIGHT-20260807-0001` 为 `consumed-blocked`；历史准备 `E3-PHYS-PREFLIGHT-20260806-0002` 为 `superseded-unexecuted`），并取得 `reviewed-pass/pass`；其他结果保持 E8 `CLOSED`。
