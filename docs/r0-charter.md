...
| NetBird | v0.74.6，commit `3a2f773d655d88d16ed953fc2a114a4e690a1b08` | R0 固定；工程集成尚未验证 |
| Go 工具链 | 1.25.12 | R0 固定；目标 ABI 构建尚未验证 |
...
用户已决定当前阶段真机暂不可连接。该限制使 R0 的具名真机准备和 HDC 闭环未满足，因此 R0 保持“进行中/未退出”。

研究期允许提前开展 R1 ABI 预探针和 Emulator HAP smoke，用于尽早发现 ABI、libc、Go、依赖、构建、调试签名、安装、启动、日志与卸载问题。此例外产生的结果仅为研究证据，不构成 R1 退出、NetBird 可行性、VPN 可行性、产品支持或发布证据。

该例外不改变 R2 必须在 arm64 具名真机复现的退出要求，不改变 R3 及以后阶段的真机门，也不改变支持矩阵、支持声明、渠道最终制品回归和发布声明必须基于具名真机证据的要求。真机到位后仍须补齐 R0 和 R1 的真机条件，再按路线图门序推进。
...
- 真机当前不可连接，HDC 安装、启动、日志、卸载闭环尚未建立。
- 普通第三方 VPN、SysCap、虚拟接口 fd、`protect`、后台生命周期和真实流量均无真机证据。
- NetBird v0.74.6、Go 1.25.12、固定依赖和跨语言边界尚未在目标 ABI 与真机验证。
...
- [x] 记录真机暂不可连接及研究期执行例外。
- [ ] 锁定具名量产设备、完整系统版本、架构、SDK/SysCap 依据和完整支持目标元组。
- [ ] 在具名真机完成 HDC 安装、启动、日志与卸载闭环。
