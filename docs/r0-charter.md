...
- `E3-PHYS-PREFLIGHT` 的历史复合输入冻结已完成，故 R0 checklist 对应历史输入项仍为 `[x]`；initial live 因 build drift 为 `reviewed-pass/blocked`。rebind `EV-E3-PHYS1REBIND7-20260806-0001` 已完成；`ADJ-20260806-0003` 冻结新元组并准备新 campaign；host reverify PASS，当前 `plan_status: ready`（仅 host 输入 ready）。R0 的渠道及其他完整支持维度仍未锁定，不能从输入冻结或证据审查通过推导 R0 退出。
