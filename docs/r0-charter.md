...
- `E3-PHYS-PREFLIGHT` 的历史复合输入冻结已完成，故 R0 checklist 对应历史输入项仍为 `[x]`；API23 initial live 因 build drift 为 `reviewed-pass/blocked`；API26 0001/0002 live 均为 `consumed-blocked`。rebind `EV-E3-PHYS1REBIND7-20260806-0001` 已完成；`ADJ-20260806-0003` 冻结新元组；host reverify PASS；`ADJ-20260806-0004` build 确认 PASS（仅 build-confirm）；`ADJ-20260807-0002` 中文完整重跑 0002 已 `consumed-blocked`；当前 `plan_status: blocked-awaiting-adjudication`（无 auto retry/新 ID/设备命令授权；禁 HDC）。R0 的渠道及其他完整支持维度仍未锁定，不能从输入冻结或证据审查通过推导 R0 退出。
...
- [x] 按 `ADJ-20260806-0003` 冻结 HarmonyOS 7 新元组并分配准备用新 campaign/evidence ID；host reverify PASS（HAP/signature/profile/member hashes 未变；不主张安装兼容性）；设备执行须再次确认；后续 API26 0001/0002 live 均为 `consumed-blocked`；当前 `plan_status: blocked-awaiting-adjudication`。
