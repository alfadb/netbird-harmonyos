...
- **当前准备身份（非执行）**：campaign ID `E3-PHYS-PREFLIGHT-20260807-0002`；evidence ID `EV-E3-PHYS1API26-20260807-0002`；target code 仍为 `PHYS1API26`。完整场景 1–7；prior blocked 显式绑定 `EV-E3-PHYS1API26-20260807-0001`。
...
- **变更项**：runner 人工提示中文化（protocol usability）；因此 **必须** 新 commit、新仓外 commit-bound freeze、dry-run、device-ready 后再谈 Live。不得把未提交 runner 中文 diff 当作已冻结输入。
- **明确禁止**：本登记期间不得执行 HDC 或任何设备端命令；不得从场景 5 局部重放 prior blocked campaign；不得复用 `consumed-blocked` 的 0001 ID 或 `superseded-unexecuted` 的 20260806 准备 ID；不得以 prior blocked 主张 `infrastructure-blocked-retry-1`；不得把本授权写成 E3/campaign pass 或设备行为 retry；不得在 commit/freeze/dryrun/device-ready 完成前启动 Live。
- **当前状态**：`plan_status: pending-commit-freeze`（host tuple/HAP ready；runner 中文可用性修正尚在 worktree、未 commit/freeze；需新 commit → freeze → dry-run → device-ready 后，再经设备执行确认才可 Live）。E3 未关闭，E8 保持 `CLOSED`。
- **已评估替代方案**：局部 scenario5 重放会破坏 campaign 完整性且被 seal 明确禁止；把 prior blocked 当 infra retry 会错误放宽纪律；改写 prior measured 会破坏可追溯性；在 runner 中文修正未 commit/freeze 前直接 Live 会跳过输入门。故采用“prior blocked 公开登记 + 新 ID 完整 1–7 + 中文 usability 修正 + pending commit/freeze”。
- **R0/SLO/补丁预算影响**：不改变首目标候选、必选功能、任何质量/安全/性能/隐私 SLO、错误预算或补丁上限；当前补丁计数不变。R0 checklist 中“执行 campaign 并取得 `reviewed-pass/pass`”仍未完成。
- **重跑范围**：完整场景 1–7 的新 campaign（新 ID）；不改写 prior blocked measured；不重跑 rebind/build-confirm discovery。
- **生效条件**：用户直接批准后立即约束文档与 host 准备身份；Live 仍须 clean worktree、含中文 runner 的 commit、完整仓外 freeze、dry-run 与设备再确认。
- **回退条件**：用户撤销授权、复用 consumed-blocked ID、局部 scenario5 重放、未授权 HDC、改写 prior measured、或把本记录写成 E3 pass / 设备行为 retry 时，立即停止并以新的动态调整记录重新界定。
- **审查状态**：路线授权由用户于 2026-08-07 直接批准；prior blocked seal 已由 isolated `anthropic/claude-sonnet-5` 审查为 0 blocker/0 major；新 campaign 尚未执行，E8 聚合审查未通过，E8 `CLOSED`。
