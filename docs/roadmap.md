...
- **当前状态**：`plan_status: blocked-awaiting-host-rebuild`。0002 已 Live 并登记为 [`EV-E3-PHYS1API26-20260807-0002`](evidence/e3-physical-preflight-api26-0002-2026-08-07.md)（`reviewed-pass/blocked`，`consumed-blocked`；S1/S4 pass，S2/S3/S5/S6/S7 blocked；cleanup `verified-clean`；双审查 0 B/5 M；opus timeout attempt-not-counted）。`ADJ-20260807-0003` 已批准 host process terminal probe 与 Settings 应用信息强制停止撤销路径（runner 变更完成，见该记录）；E3 未关闭，E8 保持 `CLOSED`。host 重建完成前无 auto retry/新 ID/设备命令授权。
...
- **调整内容**（runner 决策字段与判定规则）：
  ...
  - 字段：`terminal_mode` = `callback-post-fd|strict-process-boundary|settings-app-info-force-stop`；`process_final_state_probes`（time、status、consecutive count）；`bundle_present_during_probe`；`settings_vpn_page_observation_only=true`；`force_stop_confirmed`；`clean_reactivation_proof`。错误/unknown 一律 blocked；所有 HDC 输出仍走脱敏/raw governance。
