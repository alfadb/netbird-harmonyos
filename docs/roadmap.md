...
2026-08-30，第四次路线级 T0（`anthropic/claude-opus-5`、`xai/grok-4.6`、`openai/gpt-5.6-sol` 三席）对 [T0 材料包](t0-native-nx-gates-materials.md) 六问表决、用户批准后形成 [native N1-Nx 门治理决议](native-nx-governance.md)：路线 A（ArkTS 壳 + C++ NAPI 薄桥 + 单一 native core）为唯一分阶段可行性路线（B/E 按两席交叉取证关闭——phone 无公开进程启动/fd 传递机制，华为官方 FAQ 明文禁 fork）；N1-Nx 七门骨架绑定条款生效（N1a/N1b、N2a/N2b 拆分、fd 合同实测建立、N6 覆盖 R0 必选功能全文、判据预注册）；E8 OPEN 的 Go/E1 前提替换为 N6 `reviewed-pass/pass`、E1 转 dormant（三路触发器：Go 正式 release、NetBird 正式采用新工具链、平台 loader 侧变化）；N1-N7 物理工作按受治理例外在 E8 `CLOSED` 期间进行；许可法律评估为 N3 硬前置；bundle 排除零预授权（protect 实测不可达时停止并返回 T0）。本文其余旧"E8 需 E1 Go pass"表述按本决议理解。
...
- **E3-PHYS-PREFLIGHT 预检已完结（2026-08-30）**：[`AUTH-E3-PHYS1API26-20260829-0001`](evidence/e3-physical-preflight-authorization-2026-08-29-0001.md) 完整执行 13 门，其 Live（[证据](evidence/e3-physical-preflight-api26-20260829-0001.md) `reviewed-pass/pass`，2026-08-30 实测窗口）**S1-S7 全部 machine-verified pass——项目首次完整 pass**（fd 所有权、进程边界、deny 路径、Settings 撤销、冲突码 `2203002`、收尾清理）；S7 前置校准实证生效。AUTH `consumed-pass`、无后继 AUTH。范围仅冻结元组 E3 可达性，不等于 E4-E7/产品/数据面结论；E8 保持 `CLOSED`（开放需独立聚合审查与其余门，含当前基线 E1）。历史 pair（20260828-0001 consumed-blocked、20260825-0001 tuple-drift-retired、20260817-0002 order-invalid-retired、20260817-0001 operation-invalid-retired 及更早）均不得复用。后续路线由用户另行治理。
