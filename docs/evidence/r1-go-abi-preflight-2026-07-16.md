...
当前总门为 `CLOSED`，E0 的普通 `EntryAbility` 仍被 `10106102` 阻塞，NetBird 官方 Go loader 的 initial-exec TLS 路径失败，E3-E7 VPN runtime 未验证。此后 x86_64 Emulator 的 PASS 与 FAIL 均不得外推 arm64 或真机；E8 未 `OPEN` 前禁止任何真机执行，这是真机投入顺序而非 arm64 或真机负面技术结论。arm64 ABI、硬件、渠道、能耗和长稳等 Emulator 客观不能执行项只登记为后续真机专属，不计入 Emulator 总门，也不得据此提前真机；C-only 证据可先行，但不能单独打开 E8 或退出 R0/R1/R2。
