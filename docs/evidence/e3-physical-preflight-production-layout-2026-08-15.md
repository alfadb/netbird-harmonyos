...
用户于 2026-08-17 跨日确认 0003 gate 1-6 pass、gate 7 role mismatch blocked，并批准保留其全部历史对象、迁移到 AUTH/pair 20260817-0001 的完整 13 门。0004 S5 与 0005 S6 B 的最小脱敏 production-derived fixture 继续只作 provenance；当前执行常量绑定 `AUTH-E3-PHYS1API26-20260817-0001` 及 pair `E3-PHYS-PREFLIGHT-20260817-0001` / `EV-E3-PHYS1API26-20260817-0001`，reviewer role exact 为 `isolated-anthropic-claude-opus-5-reviewer`。历史 AUTH/pair 均不可回溯或复用。当前 host-only registration 禁止任何 HDC 可执行文件或子命令、设备、新 pair audits/freezes/records、TargetBindingConfirm、DryRun、Live、commit 与 push。
