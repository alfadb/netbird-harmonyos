...
用户于 2026-08-17 批准保留 0001 全部仓外历史对象并迁移到 AUTH/pair 20260817-0002 的完整 13 门。0004 S5 与 0005 S6 B 的最小脱敏 production-derived fixture 继续只作 provenance；执行常量最终绑定 `AUTH-E3-PHYS1API26-20260829-0001`（`consumed-pass`，2026-08-30 预检完结，无后继 AUTH），reviewer role exact 为 `isolated-anthropic-claude-opus-5-reviewer`。S6/reviewer/source/HAP 逻辑未改，历史 AUTH/pair 均不可回溯或复用。后续任何新物理 campaign 须新的用户治理/新 AUTH。
