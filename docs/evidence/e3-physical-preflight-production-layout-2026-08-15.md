...
用户于 2026-08-16 明确选择「继续：实现 B 授权分支、生产证据回归并执行新一轮完整治理」。0004 S5 与 0005 S6 B 的最小脱敏 production-derived fixture 已用于新 AUTH `AUTH-E3-PHYS1API26-20260816-0001` 及新 pair `E3-PHYS-PREFLIGHT-20260816-0001` / `EV-E3-PHYS1API26-20260816-0001` 的当前常量迁移；0005 仍只保留为 sealed production provenance，不回溯或复用其 AUTH/pair。批准范围仍不包含 HDC/设备/Live；当前 registration + pair migration 在 0B0M 审查和 host tests 通过后，可为形成 gate 1 所需 exact clean HEAD 提交/推送，campaign evidence 不得提前提交。
