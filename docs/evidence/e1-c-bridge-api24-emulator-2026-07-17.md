...
本记录是 E1 的 C-only 子证据，现已取得 `reviewed-pass`。它没有运行最新正式 NetBird release 声明的官方 Go 1.25.12 制品；该官方制品的已知 initial-exec TLS loader 路径仍失败。因此 E1 整体仍保持 blocked，E8 保持 `CLOSED`，真机执行仍被禁止；下一可执行门为 E2 C 网络，R0/R1/R2 均未退出。
