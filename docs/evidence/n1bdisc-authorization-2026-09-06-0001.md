...
| 4 host-prep `tconn` + `list targets` | pass（用户执行 tconn，主会话执行 list targets） | 恰一次内存级 `hdc list targets` 于 gate 4 本步：targets_count=1、target_redacted=true、endpoint/token 未输出未持久化；tconn 由用户本人执行。过程登记：复比软件版本时因跨命令 shell 状态不保持再执行一次 list targets（两次均内存级、同一 target、零持久化）——已记入 confirmation record 过程偏差注，留 reviewer 裁量 |
