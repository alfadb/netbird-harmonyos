# E3-PHYS-PREFLIGHT 物理设备执行授权登记（2026-08-14 · 0002）

最后核验：2026-08-14

本文登记用户（直接人类决策者）于 2026-08-14 晚显式批准的 **新** `E3-PHYS-PREFLIGHT` 物理设备执行授权（`AUTH-E3-PHYS1API26-20260814-0002`），**取代** [`AUTH-E3-PHYS1API26-20260814-0001`](e3-physical-preflight-authorization-2026-08-14-0001.md) 的未完成 Live 执行：该 AUTH 的顺序门 1–12 全部通过、Live 已启动并正常执行至 S2，但 S2 被 runner 误判为 `platform-marker-missing` 而 seal 为 **blocked** 证据（`overall=blocked`、7 场景 blocked/not-run、cleanup `verified-clean`、`is_evidence=true`、seal 完整）。**事后证据分析定位真因：runner marker 检测口径缺陷（按 entry 进程 tag 过滤，漏掉 `:vpn` 子进程行），设备侧无故障**——S2 设备侧 Allow→create 全链路实际成功（详见「0001 Live 事实定性修正」节）。该 sealed blocked live 证据占用 0001 候选 pair `E3-PHYS-PREFLIGHT-20260814-0001` / `EV-E3-PHYS1API26-20260814-0001`——**旧 pair 因此已消费，不得复用**。0001 的历史登记记录 **正文保留、不改写**；本授权 **新建** 候选 pair `E3-PHYS-PREFLIGHT-20260814-0002` / `EV-E3-PHYS1API26-20260814-0002`（attempt=initial、retry N/A）。本文是当前唯一生效的授权登记。

> **状态注记**：本文已获用户最终批准（2026-08-14 晚，聊天确认，用户明确"继续"即批准），`authorization_status: granted`。批准前本登记不构成可执行授权；批准后本登记是当前唯一生效的授权登记。

用户本次显式批准的完整事实（直接人类决策）：

1. **新授权（因 0001 Live S2 blocked 真因已定位为 runner marker 检测口径缺陷）**：2026-08-14 晚用户批准（"继续"）启动新 campaign，授权修复 runner marker 检测缺陷 + 新 pair 重跑，授权新 AUTH `AUTH-E3-PHYS1API26-20260814-0002` 与 **新建** 候选 pair `E3-PHYS-PREFLIGHT-20260814-0002` / `EV-E3-PHYS1API26-20260814-0002`；`attempt=initial`、retry N/A。
2. **0001 Live 事实定性修正（证据分析结论，非设备/执行问题）**：0001 AUTH 的顺序门 1–12 步全部完成，Live 已启动并正常执行至 S2。**S1 pass**（machine-cleanup-baseline-and-install）。**S2 设备侧 Allow→create 全链路成功**：hilog 观测到 `VPN_ONCREATE` / `VPN_CREATE_BEGIN` / `VPN_FD_SNAPSHOT`（`fd=31`、`marker=CREATE_ACCEPTED`）/ `VPN_CREATE_RESOLVED`（`accepted=true`）；UI 显示 "Start resolved"；状态栏 layout dump 含 `status_bar_vpn` 图标节点；`net_speed` 636→200 B/s。**但 runner marker 匹配口径按 entry 进程 tag 过滤，漏掉 `:vpn` 子进程行**（tag `.alfadb.netbird.e3physvpna:vpn` 截断形态，hilog 丢弃 `cn.` 前缀）→ 误判 `platform-marker-missing:create-terminal-missing-after-Allow`。S3-S7 `not-run-after-runner-failure`；exit 2；cleanup `verified-clean`（A/B 无残留、staging 无残留）。**该 blocked 不是设备/执行失败，不构成任何 retry 依据；新授权按 initial 全新开始**。
3. **旧 pair 已消费、不得复用**：`E3-PHYS-PREFLIGHT-20260814-0001` / `EV-E3-PHYS1API26-20260814-0001` 被 0001 的 sealed blocked live 证据占用（证据根 `$HOME/harmonyos-signing/netbird-e3/evidence-live-20260814-0001`，`is_evidence=true`、`execution_mode=live`、`overall=blocked`、cleanup `verified-clean`、seal 完整），**已消费**；该证据 **保留不改写**、不作为 retry 依据，其正式 evidence 登记（reviewed 审查）由 campaign 后独立流程处理，不在本登记内。本授权 **新建** pair，不沿用、不复用任何旧 ID（含 0001 pair、20260813 pair、20260810 pair）。
4. **修复内容（写入本文，见「runner marker 检测修复记录」节）**：新增 `get_bundle_from_hilog_tag` / `test_line_correlated`——tag 提取接受 entry、`:vpn` 截断、`:vpn` 完整三形态，**不按 pid 过滤**；全部 requestId 关联点改用新口径；selftest 新增 6 用例（93/93 全过）；capture tag 过滤核查无需放宽（`hilog -T E3PhysVpn` 已覆盖 `:vpn` 行）。
5. **冻结值复用与常量迁移**：signed A/B HAP 复用 `freeze/f44be17-final/artifacts/{a,b}/`（A `3a98ad68…` size `106210`、B `1adfa966…` size `106212`）；HDC `03123a78…` / `3.2.0d`；目标元组 `PLA-AL10` / `PLA-AL10 7.0.0.100(SP8C00E32R7P2)` / API `26` / `aarch64` / `arm64-v8a` / 别名 `PHYS-1`；freeze 决策字段与 0001 blocked freeze 一致。**Python 三文件字节已因治理常量迁移（AUTH ID 与候选 pair）重新冻结**（见 `repo_bytes`；0001 历史字节随 0001 登记保留）。
...
7. **设备连接纪律沿用 + 内网 endpoint 边界变更**：执行 host 仍为当前 Linux Pod；沿用「恰一次 `hdc tconn` + 恰一次内存级 `hdc list targets`」host-prep 窄例外；**变更**：20260814 会话内，endpoint 内网值允许进入聊天（用户已授权会话内可随时 `hdc list targets`），但 endpoint/token 仍 **不落盘、不写入任何记录/freeze/evidence**（详见「设备连接纪律」节）。
8. **治理语义沿用 0001 授权**：13 步顺序门结构原样沿用（见 `linux_gate_order`）；E8 保持 `CLOSED`；Live 前提交/推送授权沿用（runner + 治理变更审查后提交，campaign evidence 不提前提交）；blocked 后停止不重试。
...
```yaml
authorization_id: AUTH-E3-PHYS1API26-20260814-0002
supersedes: AUTH-E3-PHYS1API26-20260814-0001 # its Live S2 was misjudged platform-marker-missing by a runner marker-detection defect (entry-process-tag filtering missed the :vpn subprocess lines; device-side Allow->create chain actually succeeded) and sealed blocked: overall=blocked, 7 scenarios blocked/not-run, cleanup verified-clean, is_evidence=true, seal complete at $HOME/harmonyos-signing/netbird-e3/evidence-live-20260814-0001; the 0001 record is preserved unchanged, not rewritten; the 0001 candidate pair E3-PHYS-PREFLIGHT-20260814-0001 / EV-E3-PHYS1API26-20260814-0001 is CONSUMED by that sealed blocked live evidence and must never be reused
...
is_evidence: false
authorization_status: granted # user final approval 2026-08-14 evening via chat confirmation ("继续"); see 用户批准记录 section
...
repo_bytes: # governance-constant migration re-freeze: AUTH ID + candidate pair constants migrated in the Python trio (0001 frozen bytes 36e8b897…/e39f7103…/3ebd889b… remain recorded under the 0001 AUTH, not applicable here); HAP/hdc/target-tuple/decision-field values reused from the 0001 AUTH
  python_port: frozen # Python 3 semantic-equivalent port of the 0001 AUTH with the marker-detection fix (ADJ-20260814-0002 C6), three-file bytes re-frozen after the fix; the PowerShell trio remains in-repo unchanged as historical input
    runner_sha256: 33a3b45480d3af2ca7dfe5814ccc6a43157311be4a44f2a240b3ca3c9380d58e # e3-phys-preflight-campaign.py (AUTH_ID/candidate pair constants migrated to this AUTH; get_bundle_from_hilog_tag/test_line_correlated added; re-frozen)
    freeze_example_sha256: 3d5f019499ca91a18be96ca1ea47f91bbb2e09712482aaae726bde7552dcf630 # e3-phys-preflight-freeze.example.json (authorization_id/pair updated to this AUTH)
    selftest_sha256: 3a32e17133cc8a62a794862a207033fc0a511ed154e13e32dc2db57f4c0b053c # tests/e3-phys-preflight-runner-selftest.py (positive fixtures migrated; 6 new :vpn tag-form cases; OLD_AUTH_ID negative mutants kept)
  signed_hap_frozen: # reused from f44be17-final (identical to the 0001 AUTH); path $HOME/harmonyos-signing/netbird-e3/freeze/f44be17-final/artifacts/{a,b}/
...
candidate:
  campaign_id: E3-PHYS-PREFLIGHT-20260814-0002 # NEW pair created by this AUTH
  evidence_id: EV-E3-PHYS1API26-20260814-0002
  attempt: initial # this AUTH fixes attempt=initial with retry.basis/infrastructure_reason=N/A; the generic infrastructure retry branch never applies to this AUTH path; the 0001 S2-blocked Live is NOT a retry basis (runner marker-detection defect, not infrastructure failure)
  ...
  live: false
  consumed: false
  note: NEW candidate identity created by this authorization. The PRIOR pair E3-PHYS-PREFLIGHT-20260814-0001 / EV-E3-PHYS1API26-20260814-0001 (carried by AUTH-E3-PHYS1API26-20260814-0001) is CONSUMED: the 2026-08-14 Live was sealed blocked by the runner at $HOME/harmonyos-signing/netbird-e3/evidence-live-20260814-0001 (scenario-results.json record_status=collected / overall=blocked / verdict=blocked, execution_mode=live, is_evidence=true, cleanup verified-clean; seal complete) and must never be reused. That sealed blocked live evidence is PRESERVED UNCHANGED, is NOT a retry basis (runner marker-detection defect: entry-process-tag filtering missed the :vpn subprocess lines; device-side Allow->create chain actually succeeded — see 0001 Live 事实定性修正), and its formal evidence registration (reviewed) is handled by a separate post-campaign flow, not in this registration. new-candidate-id-consumption-audit-1 (PENDING): fixed out-of-repo log $HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260814-0002/audit/new-pair-id-consumption-audit-1.txt (+ .sha256). new-candidate-id-consumption-audit-2 (PENDING): fixed out-of-repo log $HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260814-0002/audit/new-pair-id-consumption-audit-2.txt (+ .sha256). ANY occupying live/evidence/seal record for the new pair => STOP and await user-authorized new IDs, never silently switch or rewrite records
linux_gate_order:
  ...
  - candidate-id-consumption-audit-1 # NEW pair E3-PHYS-PREFLIGHT-20260814-0002 / EV-E3-PHYS1API26-20260814-0002 must be unoccupied; hash-recorded; does NOT repeat the consumed prior-pair audit; fixed log: $HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260814-0002/audit/new-pair-id-consumption-audit-1.txt (+ .sha256)
  ...
  - candidate-id-consumption-audit-2 # second external audit on the frozen HEAD before selftest/DryRun/Live; hash-recorded; fixed log: $HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260814-0002/audit/new-pair-id-consumption-audit-2.txt (+ .sha256)
  - selftest-hdc0
...
retry_policy: no-auto-retry; this AUTH AUTH-E3-PHYS1API26-20260814-0002 fixes attempt=initial and retry.basis/infrastructure_reason=N/A for TargetBindingConfirm and for every ready Live/DryRun consuming its confirmation; the 0001 S2-blocked Live (sealed blocked, is_evidence=true) is NOT a retry prior — its cause was a runner marker-detection defect (entry-process-tag filtering missed the :vpn subprocess lines; device-side Allow->create chain actually succeeded), not an infrastructure allowlist reason, and the runner-enforced generic infrastructure retry branch (attempt=infrastructure-blocked-retry-1, allowlist hdc-usb-interruption|collection-storage-failure|runner-host-failure, frozen matching Live blocked evidence prior) can never enter this AUTH path; ANY gate failure stops the campaign with no retry and no ID switch; any later attempt requires new governance and a new authorization
...
| 字段 | 值 |
| --- | --- |
| AUTH ID | `AUTH-E3-PHYS1API26-20260814-0002`（取代 0001 AUTH 的未完成 Live 执行；0001 历史登记保留不改写） |
| 授权者 | 用户（直接人类决策者），2026-08-14 晚（已批准；批准方式：聊天确认，用户明确"继续"） |
| authorization_status | `granted`（2026-08-14 晚用户聊天确认批准） |
| 事故（授权起因） | 0001 AUTH 的 Live S2 被 runner 误判 `platform-marker-missing` → seal 为 blocked 证据；**真因：runner marker 检测口径缺陷（按 entry 进程 tag 过滤，漏掉 `:vpn` 子进程行），设备侧无故障**（S2 设备侧 Allow→create 全链路实际成功） |
...
| runner | Python 3 版（**修复 + 常量迁移后重新冻结**）：runner `33a3b45480d3af2ca7dfe5814ccc6a43157311be4a44f2a240b3ca3c9380d58e`、freeze example `3d5f019499ca91a18be96ca1ea47f91bbb2e09712482aaae726bde7552dcf630`、selftest `3a32e17133cc8a62a794862a207033fc0a511ed154e13e32dc2db57f4c0b053c`；0001 AUTH 历史字节（runner `36e8b897…`、selftest `e39f7103…`、freeze example `3ebd889b…`）随 0001 登记保留，不再适用本 AUTH |
| 目标元组（不变，冻结于 `ADJ-20260806-0003`） | `PLA-AL10` / `PLA-AL10 7.0.0.100(SP8C00E32R7P2)` / API `26` / `aarch64` / `arm64-v8a` / 设备别名 `PHYS-1` |
| candidate（**新建**） | `E3-PHYS-PREFLIGHT-20260814-0002` / `EV-E3-PHYS1API26-20260814-0002`，未 Live、未消耗；`attempt=initial`、retry N/A；执行前须两次 ID 消费审计（audit-1/audit-2，均 hash 记录） |
...
| 旧 pair（0001）处置 | `E3-PHYS-PREFLIGHT-20260814-0001` / `EV-E3-PHYS1API26-20260814-0001` **已消费**：0001 Live 的 sealed blocked 证据（`$HOME/harmonyos-signing/netbird-e3/evidence-live-20260814-0001`，`record_status=collected`/`overall=blocked`/`verdict=blocked`、`execution_mode=live`、`is_evidence=true`、cleanup `verified-clean`、seal 完整）；**保留不改写、不作为 retry 依据**；正式 evidence 登记（reviewed）由 campaign 后独立流程处理；**不得复用**（20260813 pair、20260810 pair 同样已消费，不得复用） |
...
| runner marker 检测修复 | 新增 `get_bundle_from_hilog_tag` / `test_line_correlated`（tag 三形态、不按 pid 过滤）；全部 requestId 关联点改用；selftest 新增 6 用例（93/93 全过）；capture tag 过滤无需放宽 |
...
执行 host 为当前 Linux Pod（与 0001 AUTH 相同，无迁移）。本登记时点 host-only 只读复测事实：
...
| 仓外 freeze | `freeze/f44be17-final/` 存在冻结 artifacts（A/B HAP hash 已复核一致，见授权摘要）；**20260814-0002 全新 blocked / ready freeze 尚未生成**（必须同步新 pair + 新 AUTH ID 字段，不得复用 0001 冻结文件） |
| 旧 live 证据根 | `$HOME/harmonyos-signing/netbird-e3/evidence-live-20260814-0001` 为 **sealed blocked 证据**（0001 AUTH 的 Live；`is_evidence=true`、`execution_mode=live`、`overall=blocked`、cleanup `verified-clean`、seal 完整）；**保留不改写、不作为 retry 依据**；本授权新 Live 使用 **新证据根**（`evidence-live-20260814-0002`，见顺序门模板），不得与旧证据根混用 |
...
| 新 pair 消费审计 | audit-1 / audit-2 **均未执行**（固定仓外日志路径见「两次新 ID 消费审计」节：`$HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260814-0002/audit/new-pair-id-consumption-audit-{1,2}.txt` + `.sha256`） |
| 0001 旧 pair 消费 | **已消费**：sealed blocked live 证据占用（见授权摘要「旧 pair 处置」行），**不得复用**；其消费事实由本登记直接记录（0001 顺序门 audit-1/audit-2 已执行，Live seal 完整） |
| HDC 进程数 | 0（未启动任何 HDC；仅定位与文件 hash/version，未执行） |
| 设备命令 | 未发出（本登记期间禁止） |
| 仓库 HEAD | 工作区基于 `main`（HEAD `02eeb7f`，待本登记 commit 后以最终 HEAD 冻结 `code_sha`；本任务禁止提交/推送；当前工作区含三文件未提交修改：runner / selftest / freeze example，即本 AUTH 的修复 + 常量迁移字节） |
...
## 0001 Live 事实定性修正（证据分析结论）

**事故事实（如实记录）**：0001 AUTH（`AUTH-E3-PHYS1API26-20260814-0001`）的顺序门 1–12 步全部完成，Live 已启动并正常执行至 S2（2026-08-14 19:27:34–19:31:56 +08:00）。**S1 pass**（machine-cleanup-baseline-and-install）。**S2 被 runner 判为 blocked**（`platform-marker-missing:create-terminal-missing-after-Allow`——用户点击 Allow 后机器未检测到 VPN create terminal 标记）；S3-S7 `not-run-after-runner-failure`；exit 2；cleanup `verified-clean`（A/B 无残留、staging 无残留）。runner 按失败路径将本次 Live seal 为 blocked 证据（`overall=blocked`、7 场景 blocked/not-run、cleanup `verified-clean`、`is_evidence=true`、seal 完整）。证据根 `$HOME/harmonyos-signing/netbird-e3/evidence-live-20260814-0001`。

**证据分析结论（定性修正）**：**设备侧无故障，S2 设备侧 Allow→create 全链路实际成功**：

- hilog 观测到完整 create 终端链：`VPN_ONCREATE` → `VPN_CREATE_BEGIN` → `VPN_FD_SNAPSHOT`（`fd=31`、`marker=CREATE_ACCEPTED`）→ `VPN_CREATE_RESOLVED`（`accepted=true`）；
- UI 显示 "Start resolved"；
- 状态栏 layout dump 含 `status_bar_vpn` 图标节点；
- `net_speed` 636→200 B/s（VPN 通道建立后流量特征）。

**误判根因（runner marker 检测口径缺陷）**：runner 的 marker 匹配按 **entry 进程 tag** 过滤（`cn.alfadb.netbird.e3physvpna`），而 `:vpn` Extension 子进程的 create/destroy 终端标记行 tag 为 **截断形态** `.alfadb.netbird.e3physvpna:vpn`（hilog 丢弃 `cn.` 前缀），且这些行 `requestId=missing`（子进程不携带 UI requestId）——按 entry tag + requestId 匹配的旧口径 **漏掉全部 `:vpn` 子进程行**，导致误判 `platform-marker-missing`。

**定性**：非设备问题、非执行失败、非基础设施故障——纯属 runner 检测口径缺陷。该 blocked **不构成任何 retry 依据**（retry 白名单 `hdc-usb-interruption`/`collection-storage-failure`/`runner-host-failure` 均不命中；且 generic retry 分支不进入本 AUTH 路径），新授权按 `attempt=initial` 全新开始。

## runner marker 检测修复记录（ADJ-20260814-0002 C6）

**修复内容（本授权强制生效）**：

1. **新增 `get_bundle_from_hilog_tag(text)`**：从 hilog 行的 **tag 路径组件** 提取所属 bundle，接受 E3 bundle 的全部进程 tag 形态——entry 进程（`cn.alfadb.netbird.e3physvpna`）、`:vpn` Extension 子进程截断形态（`.alfadb.netbird.e3physvpna:vpn`，hilog 丢弃 `cn.` 前缀）、`:vpn` 完整形态（`cn.alfadb.netbird.e3physvpna:vpn`）；只扫描 tag 路径组件、不扫描消息体；**不按 pid 过滤**。
2. **新增 `test_line_correlated(text, request_id, bundle)`**：行与目标 request 关联判定——`requestId` 字段匹配目标 request，或该行是 `:vpn` Extension 子进程行（`requestId=missing`，子进程不携带 UI requestId）且其进程 tag 以任一形态识别为对应 bundle；`:vpn` create/destroy 终端标记通过 bundle 进程 tag 关联，**不按 pid 过滤**。
3. **全部 requestId 关联点改用新口径**：runner 中所有 marker 关联判定（create 终端链、destroy 终端链、post-create fd 快照、rejection 关联等）统一改用 `test_line_correlated`。
4. **selftest 新增 6 用例（93/93 全过）**：`TestVpnSubprocessTagForms` 类覆盖——tag 三形态提取（entry / 截断 `:vpn` / 完整 `:vpn`，外来 tag 返回 None）、真实证据形态的 create 终端标记匹配（截断 tag + `requestId=missing` 的 `VPN_ONCREATE`/`VPN_CREATE_BEGIN`/`VPN_FD_SNAPSHOT` `marker=CREATE_ACCEPTED`/`VPN_CREATE_RESOLVED` `accepted=true`）、完整 `:vpn` tag 形态匹配等。实测：`Ran 93 tests`、`OK`、`PASSED=93 FAILED=0 SKIPPED=0`。
5. **capture tag 过滤核查无需放宽**：continuous capture 仍为 `hilog -T E3PhysVpn` 单流过滤——`-T` 按 tag 通道过滤，`E3PhysVpn` 通道已覆盖 `:vpn` 子进程行（子进程行 tag 为 `.alfadb.netbird.e3physvpna:vpn/E3PhysVpn`，tag 通道仍为 `E3PhysVpn`），**无需放宽 capture 过滤**。
...
## 旧 live 证据处置声明（0001 blocked live）

- `$HOME/harmonyos-signing/netbird-e3/evidence-live-20260814-0001` 是 **sealed blocked 证据**（`is_evidence=true`、`execution_mode=live`、`overall=blocked`、cleanup `verified-clean`、seal 完整，含 `campaign-seal.json` / `hash-manifest.json` / `scenario-results.json` / `operator-attestation.json` / `operator-wait-state.json` / `projection/` 等），占用 0001 候选 pair。
- 该证据 **保留不改写**（不编辑、不覆盖、不删除、不重 seal）；**不作为 retry 依据**；本授权新 campaign 不使用该证据根（新 Live 用 `evidence-live-20260814-0002`）。
...
**Python 移植三文件（runner / freeze example / selftest）——修复 + 常量迁移后重新冻结**：

- runner：`e3-phys-preflight-campaign.py` → `33a3b45480d3af2ca7dfe5814ccc6a43157311be4a44f2a240b3ca3c9380d58e`（AUTH_ID 与候选 pair 常量已迁移至本 AUTH；新增 `get_bundle_from_hilog_tag` / `test_line_correlated` 并切换全部关联点）
- freeze example：`e3-phys-preflight-freeze.example.json` → `3d5f019499ca91a18be96ca1ea47f91bbb2e09712482aaae726bde7552dcf630`（**刻意保持 `plan_status: blocked`**，仅占位，不含 campaign 哈希/路径/秘密）
- selftest：`tests/e3-phys-preflight-runner-selftest.py` → `3a32e17133cc8a62a794862a207033fc0a511ed154e13e32dc2db57f4c0b053c`（正向 fixture 迁移 + 6 个 `:vpn` tag 形态新用例；旧 AUTH ID 仅作负向 mutant 样本）
...
freeze contract 包含 `operator_trust_model` / `scenario_invalid_policy` / `layout_verification_profile` / `vpn_conflict_rejection_codes` / `process_probe_target` 等决策字段——**与 0001 blocked freeze 一致**；旧 freeze（如 20260807 candidate `INVALID-TIMELINE`、历史 runner 绑定、0001 冻结文件）一律被 runner 拒绝，任何模式（DryRun 含）均不可用于新 Live。`ready` freeze 还须携带 `independent_review_record`（`status: pass` + `reviewer_role: isolated-anthropic-claude-opus-5-reviewer` + `record_path`/`record_sha256`）绑定独立审查 record（见下）。

## 设备连接纪律（Linux Pod，host-prep 窄例外；沿用 + 内网 endpoint 边界变更）

本授权沿用 0001 AUTH 的 host-prep 窄例外（执行 host 仍为 Linux Pod），并按 2026-08-14 会话内用户授权 **放宽 endpoint 进聊天的边界**：
...
新候选 identity `E3-PHYS-PREFLIGHT-20260814-0002` / `EV-E3-PHYS1API26-20260814-0002` 必须通过 **两次外部消费审计** 且每次 **hash 记录**：

1. **new-candidate-id-consumption-audit-1（同步 trusted refs/bundle 之后、任何 `-TargetBindingConfirm`/ready freeze 之前；针对新 pair，尚未执行）**：`git fetch`/检出同步后，审计仓库（`git log --all --grep` + evidence 目录与仓外 evidence roots 全文 grep）确认没有任何 `execution_mode=live` / `is_evidence=true` / seal 记录（scenario-results/campaign-seal/hash-manifest 或独立 evidence 文档）占用新 pair `E3-PHYS-PREFLIGHT-20260814-0002` / `EV-E3-PHYS1API26-20260814-0002`；将两次 grep 的输出与时间戳记入仓外审计日志（固定路径 `$HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260814-0002/audit/new-pair-id-consumption-audit-1.txt` + `.sha256`，hash 记录）。**旧 pair（0001）的消费已由本登记直接记录**（0001 顺序门 audit-1/audit-2 已执行，Live 被 runner 误判后 seal blocked 证据占用旧 pair——见「旧 live 证据处置声明」节；20260814-0002 执行 audit-1 时不重复审计旧 pair 消费）。
2. **audit-2（最终 ready freeze 绑定 review record 之后、selftest/DryRun/Live 之前）**：在冻结 HEAD 上重跑同一审计并重新记录 hash（固定路径 `$HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260814-0002/audit/new-pair-id-consumption-audit-2.txt` + `.sha256`），确认新 pair 仍未消耗。
...
Linux Pod 必须 **按序** 满足下列门，缺一即停止并回报主会话（所有路径均为占位符；不含 target/secret；除 host-prep 一次用户本地 `hdc tconn`、一次 `hdc list targets` 与 `-TargetBindingConfirm` 的 3 条白名单设备命令外均禁 HDC）。命令模板为 Python 3 版（本 AUTH 冻结字节，修复 + 常量迁移后重新冻结；`$HARMONYOS_STABLE_HDC` 经 `source "$HOME/harmonyos/env.sh"` 取得；**所有 runner 模式执行环境必须设置 `PYTHONUNBUFFERED=1`**）：
...
2. **候选 ID 消费审计（new-candidate-id-consumption-audit-1，尚未执行）**：确认新 pair `E3-PHYS-PREFLIGHT-20260814-0002` / `EV-E3-PHYS1API26-20260814-0002` 未被任何 `execution_mode=live` / `is_evidence=true` / seal 记录占用；0001 旧 pair 的消费不在此处重复执行（已由本登记直接记录，见「旧 live 证据处置声明」节）；grep 输出与时间戳记入仓外审计日志（固定路径 `$HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260814-0002/audit/new-pair-id-consumption-audit-1.txt` + `.sha256`，hash 记录）；发现即停止，等用户新 ID 授权。
3. **blocked confirmation freeze 静态审查**：生成全新仓外 confirmation freeze（`plan_status: blocked` + `machine_fresh_confirmation.status: pending` + `independent_review_record.status: pending` + 全部外部 hash，候选 pair 为新 pair `E3-PHYS-PREFLIGHT-20260814-0002` / `EV-E3-PHYS1API26-20260814-0002`、AUTH ID `AUTH-E3-PHYS1API26-20260814-0002`；**决策字段与 0001 blocked freeze 一致**），由独立审查角色静态审查契约与角色（0 blocker/0 major）后，freeze 中 `independent_review_ready` 为 `true`——此处 `independent_review_ready=true` 只表示 **契约与角色已静态就绪**，**不是** 设备或执行就绪，也不替代 `-TargetBindingConfirm` 的机器确认；**blocked confirmation freeze 不需要独立审查 record**。
...
5. **机器 fresh confirmation（`-TargetBindingConfirm`）**：
    ...
    python3 e3-phys-preflight-campaign.py \
      -FreezeManifest $HOME/harmonyos-signing/netbird-e3/freeze/freeze-blocked-confirm-20260814-0002.json \
      -ConfirmationRecord $HOME/harmonyos-signing/netbird-e3/records/target-binding-confirmation-20260814-0002.json \
...
6. **生成全新仓外 `ready` freeze draft（同 confirmation contract）**：新对象绑定步骤 1 的精确 clean HEAD（`code_sha`）、仓内 bytes（Python 三文件哈希，**以最终 commit 为准**）与全部完整外部 hash，并绑定步骤 5 的 record：`machine_fresh_confirmation.status=pass`、`authorization_id=AUTH-E3-PHYS1API26-20260814-0002`、`record_path=<record>`、`record_sha256=<sha>`；`plan_status: ready`；**confirmation contract 与步骤 3 的 blocked confirmation freeze 字节一致**（执行核心/候选/外部输入/code/runner/HDC/角色相同），而 `preflight_inputs_frozen_at` 推进到确认与审查 ended_at 之后（完整 freeze contract hash 因此不同，属预期）。**不得** 原地改旧 `blocked` freeze。
...
11. **DryRun（host-only，非证据；对同一份最终 `ready` freeze）**：
    ...
    ```bash
    python3 e3-phys-preflight-campaign.py \
      -FreezeManifest $HOME/harmonyos-signing/netbird-e3/freeze/freeze-ready-20260814-0002.json \
      -EvidenceRoot $HOME/harmonyos-signing/netbird-e3/evidence-dry-run-20260814-0002 \
      -RawRoot $HOME/harmonyos-signing/netbird-e3/evidence-dry-run-20260814-0002.raw \
      -HapA $HOME/harmonyos-signing/netbird-e3/freeze/f44be17-final/artifacts/a/e3-phys-preflight-a-signed.hap \
      -HapB $HOME/harmonyos-signing/netbird-e3/freeze/f44be17-final/artifacts/b/e3-phys-preflight-b-signed.hap \
...
13. **单次 Live（唯一一次；只接受步骤 8 的同一份 `plan_status: ready` freeze）**：
    ...
    python3 e3-phys-preflight-campaign.py \
      -FreezeManifest $HOME/harmonyos-signing/netbird-e3/freeze/freeze-ready-20260814-0002.json \
      -EvidenceRoot $HOME/harmonyos-signing/netbird-e3/evidence-live-20260814-0002 \
      -RawRoot $HOME/harmonyos-signing/netbird-e3/evidence-live-20260814-0002.raw \
      -HapA $HOME/harmonyos-signing/netbird-e3/freeze/f44be17-final/artifacts/a/e3-phys-preflight-a-signed.hap \
      -HapB $HOME/harmonyos-signing/netbird-e3/freeze/f44be17-final/artifacts/b/e3-phys-preflight-b-signed.hap \
...
- **当前 AUTH 固定 `attempt=initial`（retry N/A）**：`-TargetBindingConfirm` 与消费本 AUTH confirmation 的 `ready` Live/DryRun 均被 runner 强制 exact candidate pair + `attempt=initial` + `retry.basis`/`infrastructure_reason=N/A`——因此 **现有 generic retry 分支（`attempt: infrastructure-blocked-retry-1`）不进入本次路径**。**任一 gate 失败即停止，不重试、不换 ID**。任何后续尝试必须 **新治理**：先取得新的路线决策并重新登记新授权；当前 AUTH `AUTH-E3-PHYS1API26-20260814-0002` 不可用于任何 retry。
- **0001 被误判 Live 不构成 retry 依据**：其 sealed blocked evidence（`is_evidence=true`）虽满足「prior_record 为 Live blocked evidence」的形式条件，但其成因为 runner marker 检测口径缺陷（entry 进程 tag 过滤漏掉 `:vpn` 子进程行；设备侧 Allow→create 全链路实际成功），不命中 retry 白名单 `hdc-usb-interruption` / `collection-storage-failure` / `runner-host-failure` 中任何一项；本 AUTH 已按新治理新建 pair 并以 `attempt=initial` 全新开始。旧 pair（`E3-PHYS-PREFLIGHT-20260814-0001` / `EV-E3-PHYS1API26-20260814-0001`）**已消费，不得复用**。
- **20260813 / 20260810 各 pair 同样已消费、不得复用**：`E3-PHYS-PREFLIGHT-20260810-0001` / `EV-E3-PHYS1API26-20260810-0001`（20260813 被中断 Live 的 sealed blocked 证据占用）与更早的 `20260808` 等 pair 一律不得复用。
...
- 本授权不豁免任何既有纪律：build drift、operator-aborted、功能 fail、scenario invalid、integrity violation、非基础设施 blocked 均不授权 retry；`consumed-blocked` / `superseded-unexecuted` / `INVALID-TIMELINE` 历史 ID 一律不得复用（含旧 pair `E3-PHYS-PREFLIGHT-20260814-0001` / `EV-E3-PHYS1API26-20260814-0001`、`E3-PHYS-PREFLIGHT-20260810-0001` / `EV-E3-PHYS1API26-20260810-0001` 与更早的 `20260808` 等 pair）。
...
5. **禁止改写历史**：0001 及更早授权/证据登记记录保留不改写；旧 pair（`20260814-0001`/`20260810`/`20260808` 等）不得复用；`$HOME/harmonyos-signing/netbird-e3/evidence-live-20260814-0001`（0001 sealed blocked 证据）**保留不改写**，其正式 evidence 登记由 campaign 后独立流程处理；本登记任务本身禁止提交/推送。
...
| 字段 | 值 |
| --- | --- |
| 批准时间 | `2026-08-14T20:xx:xx+08:00`（Asia/Shanghai；约值，落盘精确值以生成时刻为准） |
| 批准方式 | 聊天确认（用户明确"继续"） |
| 批准后 authorization_status | `granted` |

## 执行注记（2026-08-14 晚）

1. **0001 Live 事实定性修正**：证据分析确认 S2 设备侧 Allow→create 全链路成功（hilog `VPN_ONCREATE`/`VPN_CREATE_BEGIN`/`VPN_FD_SNAPSHOT` `fd=31` `marker=CREATE_ACCEPTED`/`VPN_CREATE_RESOLVED` `accepted=true`；UI "Start resolved"；状态栏 `status_bar_vpn` 图标节点；`net_speed` 636→200 B/s）；误判根因为 runner marker 匹配按 entry 进程 tag 过滤、漏掉 `:vpn` 子进程行（tag `.alfadb.netbird.e3physvpna:vpn` 截断形态）→ `platform-marker-missing`。设备侧无故障。
2. **runner marker 检测修复**：新增 `get_bundle_from_hilog_tag` / `test_line_correlated`（tag 提取接受 entry、`:vpn` 截断、`:vpn` 完整三形态，不按 pid 过滤）；全部 requestId 关联点改用；selftest 新增 6 用例，实测 **93/93 全过**（`Ran 93 tests`、`OK`、`PASSED=93 FAILED=0 SKIPPED=0`）；capture tag 过滤核查无需放宽（`hilog -T E3PhysVpn` 已覆盖 `:vpn` 行）。
3. **常量迁移与重新冻结**：runner/selftest/freeze example 治理常量迁移至 20260814-0002 系列，重新冻结（runner `33a3b454…`、selftest `3a32e171…`、freeze example `3d5f0194…`）；三文件当前为工作区未提交修改（HEAD `02eeb7f`），`code_sha` 待 commit 后冻结。
4. **按纪律停止**：0001 blocked 后不重试、不换 ID；本授权按新治理新建 pair 并以 `attempt=initial` 全新开始。E3 未关闭，E8 保持 `CLOSED`。
