# E3-PHYS-PREFLIGHT 物理设备执行授权登记（2026-08-15 · 0003）

最后核验：2026-08-15

本文登记用户（直接人类决策者）于 2026-08-15 下午显式批准的 **新** `E3-PHYS-PREFLIGHT` 物理设备执行授权（`AUTH-E3-PHYS1API26-20260815-0003`），**取代** [`AUTH-E3-PHYS1API26-20260815-0002`](e3-physical-preflight-authorization-2026-08-15-0002.md) 的未完成 Live 执行：该 AUTH 的 Live（2026-08-15 13:02:39–13:04:10 +08:00）S1 pass，但 **S2 invalid**（`layout-checkpoint-scenario-2-entry-a-layout-mismatch`——capture 布局树为锁屏页 `ScreenLockRootComponent`，设备屏幕锁定）→ `SCENARIO_INVALID` → seal 为 **invalid** 证据（`overall=invalid`、S2 invalid、S3–S7 not-run-due-to-invalid、cleanup `verified-clean`、`is_evidence=true`、seal 完整、exit 2）。该 sealed invalid live 证据占用 0002 候选 pair `E3-PHYS-PREFLIGHT-20260815-0002` / `EV-E3-PHYS1API26-20260815-0002`——**旧 pair 因此已消费，不得复用**。0002 的历史登记记录 **正文保留、不改写**；本授权 **新建** 候选 pair `E3-PHYS-PREFLIGHT-20260815-0003` / `EV-E3-PHYS1API26-20260815-0003`（attempt=initial、retry N/A）。本文是当前唯一生效的授权登记。

> **状态注记**：本文已获用户最终批准（2026-08-15 下午，聊天确认，用户回复"准备好了"），`authorization_status: granted`。批准前本登记不构成可执行授权；批准后本登记是当前唯一生效的授权登记。

用户本次显式批准的完整事实（直接人类决策）：

1. **新授权（因 0002 Live S2 设备锁屏失败）**：2026-08-15 下午用户批准（"准备好了"）：0002 Live 的 S2 因设备锁屏失败（entry capture 到 `ScreenLockRootComponent` 锁屏页面 → layout-mismatch），冻结输入无问题，用户已解锁设备并将休眠调至最长/屏幕常亮，批准 **新 pair 重跑**；授权新 AUTH `AUTH-E3-PHYS1API26-20260815-0003` 与 **新建** 候选 pair `E3-PHYS-PREFLIGHT-20260815-0003` / `EV-E3-PHYS1API26-20260815-0003`；`attempt=initial`、retry N/A。
2. **0002 Live 事实定性（证据分析结论，非设备/执行故障）**：0002 AUTH 的 Live 于 2026-08-15 13:02:39–13:04:10 +08:00 执行：**S1 pass**（machine-cleanup-baseline-and-install）、**S2 invalid**（`layout-checkpoint-scenario-2-entry-a-layout-mismatch`：capture 布局树为锁屏页 `ScreenLockRootComponent`，`expected-bundle`/`start-control`/`stop-control` 缺失，设备屏幕锁定）→ `SCENARIO_INVALID`（scenario=2、reason=layout-checkpoint-scenario-2-entry-a-layout-mismatch）→ S3–S7 `not-run-due-to-invalid`；exit 2；cleanup `verified-clean`（A/B 卸载无残留、staging 无残留、force-stop 仅作残留清理未用作 revoke）；`integrity_violations: []`。**根因：设备屏幕状态/环境问题**（Live 前设备屏幕锁定，entry capture 捕获到锁屏页）——不是冻结输入缺陷、不是 runner 缺陷、不构成任何 retry 依据；新授权按 initial 全新开始。**修复措施（写入本文）**：Live 前操作员解锁设备并保持屏幕常亮，再开始 Live。
3. **旧 pair 已消费、不得复用**：`E3-PHYS-PREFLIGHT-20260815-0002` / `EV-E3-PHYS1API26-20260815-0002` 被 0002 的 sealed invalid live 证据占用（证据根 `$HOME/harmonyos-signing/netbird-e3/evidence-live-20260815-0002`，`is_evidence=true`、`execution_mode=live`、`overall=invalid`、cleanup `verified-clean`、seal 完整），**已消费**；该证据 **保留不改写**、不作为 retry 依据，其正式 evidence 登记（reviewed 审查）由 campaign 后独立流程处理，不在本登记内。本授权 **新建** pair，不沿用、不复用任何旧 ID（含 0002 pair、0001 pair、20260813 pair、20260810 pair）。
4. **本次变更清单（写入本文，见「本次变更清单」节）**：**仅治理常量迁移**——runner / selftest / freeze example 三文件中的 `20260815-0002` → `20260815-0003`（AUTH_ID / 候选 pair / 正向 fixture / authorization_id；OLD/负向 mutant 保留历史 ID 语义）。**无 HAP 变更、无显示名变更、无 audit 脚本变更**——0001 的显示名区分（`E3 Preflight A`/`E3 Preflight B`）、Linux 构建+重签 HAP、audit 漂移修复全部原样复用。
5. **冻结值复用（全部不变）**：runner `0cc1115d…`、selftest `81b951cf…`、freeze example `c1c88a2b…`（**仅常量迁移后字节变**，新字节见 `repo_bytes`；0002 历史字节随 0002 登记保留）；HAP A `828fefed…`/`133935`、B `c9e064ed…`/`133945`（路径 `20260815-relabeled/artifacts/{a,b}/`）；HDC `03123a78…` / `3.2.0d`；目标元组 `PLA-AL10` / `PLA-AL10 7.0.0.100(SP8C00E32R7P2)` / API `26` / `aarch64` / `arm64-v8a` / 别名 `PHYS-1`；freeze 决策字段与 0002 invalid freeze 一致。
6. **修复措施沿用（防再犯，写入本文）**：Live 执行环境设置 `PYTHONUNBUFFERED=1`（禁止 stdout 块缓冲掩盖 operator 提示）；主会话监控改用 **证据目录轮询**（`operator-wait-state.json` / `scenario-results.json` 的 `updated_at` 与产物增量）而非 pane 输出空白判断；operator 提示经 `capture-pane` 转发给用户。**新增（0003）**：Live 前操作员解锁设备并保持屏幕常亮（本 AUTH 强制生效）。
...
```yaml
authorization_id: AUTH-E3-PHYS1API26-20260815-0003
supersedes: AUTH-E3-PHYS1API26-20260815-0002 # its Live (2026-08-15 13:02:39-13:04:10 +08:00) ran S1 pass, then S2 invalid (layout-checkpoint-scenario-2-entry-a-layout-mismatch: capture layout tree was the lock screen page ScreenLockRootComponent, expected-bundle/start-control/stop-control missing, device screen was locked) -> SCENARIO_INVALID -> sealed invalid: overall=invalid, S2 invalid, S3-S7 not-run-due-to-invalid, cleanup verified-clean, is_evidence=true, seal complete at $HOME/harmonyos-signing/netbird-e3/evidence-live-20260815-0002; the 0002 record is preserved unchanged, not rewritten; the 0002 candidate pair E3-PHYS-PREFLIGHT-20260815-0002 / EV-E3-PHYS1API26-20260815-0002 is CONSUMED by that sealed invalid live evidence and must never be reused
...
is_evidence: false
authorization_status: granted # user final approval 2026-08-15 afternoon via chat confirmation (user replied "准备好了"); see 用户批准记录 section
...
repo_bytes: # governance-constant migration re-freeze: AUTH ID + candidate pair constants migrated in the Python trio (0002 frozen bytes 0cc1115d…/81b951cf…/c1c88a2b… remain recorded under the 0002 AUTH, not applicable here); HAP inputs REUSED unchanged (20260815-relabeled); hdc/target-tuple/decision-field values reused from the 0002 AUTH
  python_port: frozen # Python 3 semantic-equivalent port of the 0002 AUTH (marker-detection fix ADJ-20260814-0002 C6 carried forward unchanged), three-file bytes re-frozen after the constant migration; the PowerShell trio remains in-repo unchanged as historical input
    runner_sha256: 40db928f90e3f6d0f83fc0b78e75de2dca362cd1b1384d2cd0ce40c0d3a399a8 # e3-phys-preflight-campaign.py (AUTH_ID/candidate pair constants migrated 20260815-0002 -> 20260815-0003; re-frozen)
    freeze_example_sha256: 72ae686dda9c874f4e1287786104ab7932c956c3d2816a048dfb78409292e250 # e3-phys-preflight-freeze.example.json (authorization_id/pair updated to this AUTH)
    selftest_sha256: 8b1fd2be82389984dca70c34b23347ec05a9280d8c211d0a1d8124143abd9e5f # tests/e3-phys-preflight-runner-selftest.py (positive fixtures migrated; OLD_AUTH_ID negative mutants kept with historical ID semantics)
  signed_hap_frozen: # REUSED unchanged from the 0002 AUTH (Linux-built + re-signed, relabeled display names); path $HOME/harmonyos-signing/netbird-e3/freeze/20260815-relabeled/artifacts/{a,b}/
...
candidate:
  campaign_id: E3-PHYS-PREFLIGHT-20260815-0003 # NEW pair created by this AUTH
  evidence_id: EV-E3-PHYS1API26-20260815-0003
  attempt: initial # this AUTH fixes attempt=initial with retry.basis/infrastructure_reason=N/A; the generic infrastructure retry branch never applies to this AUTH path; the 0002 S2-layout-mismatch Live is NOT a retry basis (device screen state/environment issue, not infrastructure failure)
  ...
  live: false
  consumed: false
  note: NEW candidate identity created by this authorization. The PRIOR pair E3-PHYS-PREFLIGHT-20260815-0002 / EV-E3-PHYS1API26-20260815-0002 (carried by AUTH-E3-PHYS1API26-20260815-0002) is CONSUMED: the 2026-08-15 Live was sealed invalid by the runner at $HOME/harmonyos-signing/netbird-e3/evidence-live-20260815-0002 (scenario-results.json record_status=invalidated / overall=invalid / verdict=invalid, execution_mode=live, is_evidence=true, cleanup verified-clean; seal complete; exit 2) and must never be reused. That sealed invalid live evidence is PRESERVED UNCHANGED, is NOT a retry basis (S2 layout-checkpoint-scenario-2-entry-a-layout-mismatch: capture layout tree was the lock screen page ScreenLockRootComponent, expected-bundle/start-control/stop-control missing, device screen was locked — device screen state/environment issue, now addressed by the operator unlocking the device and keeping the screen always-on before Live), and its formal evidence registration (reviewed) is handled by a separate post-campaign flow, not in this registration. new-candidate-id-consumption-audit-1 (PENDING): fixed out-of-repo log $HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260815-0003/audit/new-pair-id-consumption-audit-1.txt (+ .sha256). new-candidate-id-consumption-audit-2 (PENDING): fixed out-of-repo log $HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260815-0003/audit/new-pair-id-consumption-audit-2.txt (+ .sha256). ANY occupying live/evidence/seal record for the new pair => STOP and await user-authorized new IDs, never silently switch or rewrite records
linux_gate_order:
  ...
  - candidate-id-consumption-audit-1 # NEW pair E3-PHYS-PREFLIGHT-20260815-0003 / EV-E3-PHYS1API26-20260815-0003 must be unoccupied; hash-recorded; does NOT repeat the consumed prior-pair audit; fixed log: $HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260815-0003/audit/new-pair-id-consumption-audit-1.txt (+ .sha256)
  ...
  - candidate-id-consumption-audit-2 # second external audit on the frozen HEAD before selftest/DryRun/Live; hash-recorded; fixed log: $HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260815-0003/audit/new-pair-id-consumption-audit-2.txt (+ .sha256)
  - selftest-hdc0
...
retry_policy: no-auto-retry; this AUTH AUTH-E3-PHYS1API26-20260815-0003 fixes attempt=initial and retry.basis/infrastructure_reason=N/A for TargetBindingConfirm and for every ready Live/DryRun consuming its confirmation; the 0002 S2-layout-mismatch Live (sealed invalid, is_evidence=true) is NOT a retry prior — its cause was the device screen state/environment (capture layout tree was the lock screen page ScreenLockRootComponent, expected-bundle/start-control/stop-control missing, device screen was locked; now addressed by the operator unlocking the device and keeping the screen always-on before Live), not an infrastructure allowlist reason, and the runner-enforced generic infrastructure retry branch (attempt=infrastructure-blocked-retry-1, allowlist hdc-usb-interruption|collection-storage-failure|runner-host-failure, frozen matching Live blocked evidence prior) can never enter this AUTH path; ANY gate failure stops the campaign with no retry and no ID switch; any later attempt requires new governance and a new authorization
...
| 字段 | 值 |
| --- | --- |
| AUTH ID | `AUTH-E3-PHYS1API26-20260815-0003`（取代 0002 AUTH 的未完成 Live 执行；0002 历史登记保留不改写） |
| 授权者 | 用户（直接人类决策者），2026-08-15 下午（已批准；批准方式：聊天确认，用户回复"准备好了"） |
| authorization_status | `granted`（2026-08-15 下午用户聊天确认批准） |
| 事故（授权起因） | 0002 AUTH 的 Live S2 invalid（`layout-checkpoint-scenario-2-entry-a-layout-mismatch`：capture 布局树为锁屏页 `ScreenLockRootComponent`，`expected-bundle`/`start-control`/`stop-control` 缺失，设备屏幕锁定）→ `SCENARIO_INVALID` → seal invalid；**根因：设备屏幕状态/环境问题**（Live 前设备屏幕锁定）——不是冻结输入缺陷 |
...
| runner | Python 3 版（**常量迁移后重新冻结**）：runner `40db928f…`、freeze example `72ae686d…`、selftest `8b1fd2be…`；0002 AUTH 历史字节（runner `0cc1115d…`、selftest `81b951cf…`、freeze example `c1c88a2b…`）随 0002 登记保留，不再适用本 AUTH |
| 目标元组（不变，冻结于 `ADJ-20260806-0003`） | `PLA-AL10` / `PLA-AL10 7.0.0.100(SP8C00E32R7P2)` / API `26` / `aarch64` / `arm64-v8a` / 设备别名 `PHYS-1` |
| candidate（**新建**） | `E3-PHYS-PREFLIGHT-20260815-0003` / `EV-E3-PHYS1API26-20260815-0003`，未 Live、未消耗；`attempt=initial`、retry N/A；执行前须两次 ID 消费审计（audit-1/audit-2，均 hash 记录） |
| signed A/B HAP（**复用 0002 不变** `20260815-relabeled`） | `$HOME/harmonyos-signing/netbird-e3/freeze/20260815-relabeled/artifacts/{a,b}/`：A `828fefedba3b4ec63cb3fbe6336f4bf34266770cb29465c854f0ec94491b503d` size `133935`（`e3-phys-preflight-a-signed.hap`）；B `c9e064ed1dcf0df112c27c5f6bcc282e9a9913c63d3dfd8ce8377b1b46218a37` size `133945`（`e3-phys-preflight-b-signed.hap`） |
...
| 旧 pair（0002）处置 | `E3-PHYS-PREFLIGHT-20260815-0002` / `EV-E3-PHYS1API26-20260815-0002` **已消费**：0002 Live 的 sealed invalid 证据（`$HOME/harmonyos-signing/netbird-e3/evidence-live-20260815-0002`，`record_status=invalidated`/`overall=invalid`/`verdict=invalid`、`execution_mode=live`、`is_evidence=true`、cleanup `verified-clean`、seal 完整、exit 2）；**保留不改写、不作为 retry 依据**；正式 evidence 登记（reviewed）由 campaign 后独立流程处理；**不得复用**（0001 pair、20260813 pair、20260810 pair 同样已消费，不得复用） |
...
| 修复措施 | `PYTHONUNBUFFERED=1`；主会话改证据目录轮询；operator 提示经 `capture-pane` 转发；**新增：Live 前操作员解锁设备并保持屏幕常亮** |
| 本次变更 | **仅治理常量迁移**：runner / selftest / freeze example 三文件 `20260815-0002` → `20260815-0003`（AUTH_ID / 候选 pair / 正向 fixture / authorization_id；OLD/负向 mutant 保留历史 ID 语义）；无 HAP/显示名/audit 脚本变更（0001 变更全部原样复用） |
...
| 仓外 freeze | `freeze/20260815-relabeled/` 冻结 artifacts（A/B HAP hash 已复核一致，见授权摘要）；**20260815-0003 全新 blocked / ready freeze 尚未生成**（必须同步新 pair + 新 AUTH ID 字段，不得复用 0002 冻结文件）；旧 `freeze/f44be17-final/` 保留作为历史输入 |
| 旧 live 证据根 | `$HOME/harmonyos-signing/netbird-e3/evidence-live-20260815-0002` 为 **sealed invalid 证据**（0002 AUTH 的 Live；`is_evidence=true`、`execution_mode=live`、`overall=invalid`、cleanup `verified-clean`、seal 完整、exit 2）；**保留不改写、不作为 retry 依据**；本授权新 Live 使用 **新证据根**（`evidence-live-20260815-0003`，见顺序门模板），不得与旧证据根混用 |
...
| 新 pair 消费审计 | audit-1 / audit-2 **均未执行**（固定仓外日志路径见「两次新 ID 消费审计」节：`$HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260815-0003/audit/new-pair-id-consumption-audit-{1,2}.txt` + `.sha256`） |
...
| 设备命令 | 未发出（本登记期间禁止） |
| 仓库 HEAD | 工作区基于 `main`（HEAD `5eec425`，待本登记 commit 后以最终 HEAD 冻结 `code_sha`；本任务禁止提交/推送；当前工作区含三文件未提交修改：runner / selftest / freeze example（本 AUTH 常量迁移字节）） |
...
**事故事实（如实记录）**：0002 AUTH（`AUTH-E3-PHYS1API26-20260815-0002`）的顺序门 1–13 步全部完成，Live 于 2026-08-15 13:02:39–13:04:10 +08:00 执行。**S1 pass**（machine-cleanup-baseline-and-install）。**S2 invalid**（`layout-checkpoint-scenario-2-entry-a-layout-mismatch`）：entry capture 的布局树为 **锁屏页 `ScreenLockRootComponent`**（设备屏幕锁定），`expected-bundle` / `start-control` / `stop-control` 均缺失 → `SCENARIO_INVALID`（scenario=2、reason=layout-checkpoint-scenario-2-entry-a-layout-mismatch）；S3–S7 `not-run-due-to-invalid`；exit 2；cleanup `verified-clean`（A/B 卸载无残留、staging 无残留、force-stop 仅作残留清理未用作 revoke）；`integrity_violations: []`。runner 按失败路径将本次 Live seal 为 invalid 证据（`overall=invalid`、7 场景 invalid/not-run、cleanup `verified-clean`、`is_evidence=true`、seal 完整）。证据根 `$HOME/harmonyos-signing/netbird-e3/evidence-live-20260815-0002`。

**证据分析结论（定性）**：**设备侧无故障、runner 无缺陷、冻结输入无缺陷**。S2 布局不匹配根因为 **设备屏幕状态/环境问题**：Live 前设备屏幕锁定，entry capture 捕获到锁屏页（`ScreenLockRootComponent` 布局），而非 App A 的 entry 布局。S1 全链路（cleanup baseline + A/B install）机器验证通过。

**定性**：非冻结输入缺陷、非执行失败、非基础设施故障——纯属 **设备屏幕状态/环境** 导致 S2 entry capture 布局不匹配。该 invalid **不构成任何 retry 依据**（retry 白名单 `hdc-usb-interruption`/`collection-storage-failure`/`runner-host-failure` 均不命中；且 generic retry 分支不进入本 AUTH 路径），新授权按 `attempt=initial` 全新开始。**修复措施（写入本文，本 AUTH 强制生效）**：Live 前操作员解锁设备并保持屏幕常亮，再开始 Live。

## 本次变更清单（本授权强制生效）

1. **治理常量迁移（唯一变更）**：runner / selftest / freeze example 三文件中的 `20260815-0002` → `20260815-0003`：
   - runner `e3-phys-preflight-campaign.py`：`AUTH_ID` / `CANDIDATE_CAMPAIGN_ID` / `CANDIDATE_EVIDENCE_ID` 及 docstring 绑定描述迁移至 0003。
   - selftest `tests/e3-phys-preflight-runner-selftest.py`：正向 fixture 常量（`AUTH_ID` / `CANDIDATE_CAMPAIGN_ID` / `CANDIDATE_EVIDENCE_ID`）迁移至 0003；`OLD_AUTH_ID`（`AUTH-E3-PHYS1API26-20260810-0002`）等负向 mutant 与 `ADJ-20260814-0002 C6` 历史语义 **保留不变**。
   - freeze example `e3-phys-preflight-freeze.example.json`：`evidence_id` / `campaign_id` / `authorization_id` 迁移至 0003。
...
- `$HOME/harmonyos-signing/netbird-e3/evidence-live-20260815-0002` 是 **sealed invalid 证据**（`is_evidence=true`、`execution_mode=live`、`overall=invalid`、cleanup `verified-clean`、seal 完整、exit 2，含 `campaign-seal.json` / `hash-manifest.json` / `scenario-results.json` / `operator-attestation.json` / `operator-wait-state.json` / `projection/` 等），占用 0002 候选 pair。
- 该证据 **保留不改写**（不编辑、不覆盖、不删除、不重 seal）；**不作为 retry 依据**；本授权新 campaign 不使用该证据根（新 Live 用 `evidence-live-20260815-0003`）。
...
- runner：`e3-phys-preflight-campaign.py` → `40db928f90e3f6d0f83fc0b78e75de2dca362cd1b1384d2cd0ce40c0d3a399a8`（AUTH_ID 与候选 pair 常量迁移至 0003；其余字节与 0002 冻结值 `0cc1115d…` 语义一致）
- freeze example：`e3-phys-preflight-freeze.example.json` → `72ae686dda9c874f4e1287786104ab7932c956c3d2816a048dfb78409292e250`（**刻意保持 `plan_status: blocked`**，仅占位，不含 campaign 哈希/路径/秘密）
- selftest：`tests/e3-phys-preflight-runner-selftest.py` → `8b1fd2be82389984dca70c34b23347ec05a9280d8c211d0a1d8124143abd9e5f`（正向 fixture 迁移至 0003；`OLD_AUTH_ID` 负向 mutant 保留历史语义）
...
新候选 identity `E3-PHYS-PREFLIGHT-20260815-0003` / `EV-E3-PHYS1API26-20260815-0003` 必须通过 **两次外部消费审计** 且每次 **hash 记录**：

1. **new-candidate-id-consumption-audit-1（同步 trusted refs/bundle 之后、任何 `-TargetBindingConfirm`/ready freeze 之前；针对新 pair，尚未执行）**：`git fetch`/检出同步后，审计仓库（`git log --all --grep` + evidence 目录与仓外 evidence roots 全文 grep）确认没有任何 `execution_mode=live` / `is_evidence=true` / seal 记录（scenario-results/campaign-seal/hash-manifest 或独立 evidence 文档）占用新 pair `E3-PHYS-PREFLIGHT-20260815-0003` / `EV-E3-PHYS1API26-20260815-0003`；将两次 grep 的输出与时间戳记入仓外审计日志（固定路径 `$HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260815-0003/audit/new-pair-id-consumption-audit-1.txt` + `.sha256`，hash 记录）。**旧 pair（0002）的消费已由本登记直接记录**（0002 顺序门 audit-1/audit-2 已执行，Live 被 runner seal 为 invalid 证据占用旧 pair——见「旧 live 证据处置声明」节；20260815-0003 执行 audit-1 时不重复审计旧 pair 消费）。
2. **audit-2（最终 ready freeze 绑定 review record 之后、selftest/DryRun/Live 之前）**：在冻结 HEAD 上重跑同一审计并重新记录 hash（固定路径 `$HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260815-0003/audit/new-pair-id-consumption-audit-2.txt` + `.sha256`），确认新 pair 仍未消耗。
...
2. **候选 ID 消费审计（new-candidate-id-consumption-audit-1，尚未执行）**：确认新 pair `E3-PHYS-PREFLIGHT-20260815-0003` / `EV-E3-PHYS1API26-20260815-0003` 未被任何 `execution_mode=live` / `is_evidence=true` / seal 记录占用；0002 旧 pair 的消费不在此处重复执行（已由本登记直接记录，见「旧 live 证据处置声明」节）；grep 输出与时间戳记入仓外审计日志（固定路径 `$HOME/harmonyos-signing/netbird-e3/campaigns/E3-PHYS-PREFLIGHT-20260815-0003/audit/new-pair-id-consumption-audit-1.txt` + `.sha256`，hash 记录）；发现即停止，等用户新 ID 授权。
3. **blocked confirmation freeze 静态审查**：生成全新仓外 confirmation freeze（`plan_status: blocked` + `machine_fresh_confirmation.status: pending` + `independent_review_record.status: pending` + 全部外部 hash，候选 pair 为新 pair `E3-PHYS-PREFLIGHT-20260815-0003` / `EV-E3-PHYS1API26-20260815-0003`、AUTH ID `AUTH-E3-PHYS1API26-20260815-0003`；**决策字段与 0002 invalid freeze 一致**），由独立审查角色静态审查契约与角色（0 blocker/0 major）后，freeze 中 `independent_review_ready` 为 `true`——此处 `independent_review_ready=true` 只表示 **契约与角色已静态就绪**，**不是** 设备或执行就绪，也不替代 `-TargetBindingConfirm` 的机器确认；**blocked confirmation freeze 不需要独立审查 record**。
...
5. **机器 fresh confirmation（`-TargetBindingConfirm`）**：
    ...
    python3 e3-phys-preflight-campaign.py \
      -FreezeManifest $HOME/harmonyos-signing/netbird-e3/freeze/freeze-blocked-confirm-20260815-0003.json \
      -ConfirmationRecord $HOME/harmonyos-signing/netbird-e3/records/target-binding-confirmation-20260815-0003.json \
...
6. **生成全新仓外 `ready` freeze draft（同 confirmation contract）**：新对象绑定步骤 1 的精确 clean HEAD（`code_sha`）、仓内 bytes（Python 三文件哈希，**以最终 commit 为准**）与全部完整外部 hash，并绑定步骤 5 的 record：`machine_fresh_confirmation.status=pass`、`authorization_id=AUTH-E3-PHYS1API26-20260815-0003`、`record_path=<record>`、`record_sha256=<sha>`；`plan_status: ready`；**confirmation contract 与步骤 3 的 blocked confirmation freeze 字节一致**（执行核心/候选/外部输入/code/runner/HDC/角色相同），而 `preflight_inputs_frozen_at` 推进到确认与审查 ended_at 之后（完整 freeze contract hash 因此不同，属预期）。**不得** 原地改旧 `blocked` freeze。
...
11. **DryRun（host-only，非证据；对同一份最终 `ready` freeze）**：
    ...
    ```bash
    python3 e3-phys-preflight-campaign.py \
      -FreezeManifest $HOME/harmonyos-signing/netbird-e3/freeze/freeze-ready-20260815-0003.json \
      -EvidenceRoot $HOME/harmonyos-signing/netbird-e3/evidence-dry-run-20260815-0003 \
      -RawRoot $HOME/harmonyos-signing/netbird-e3/evidence-dry-run-20260815-0003.raw \
      -HapA $HOME/harmonyos-signing/netbird-e3/freeze/20260815-relabeled/artifacts/a/e3-phys-preflight-a-signed.hap \
      -HapB $HOME/harmonyos-signing/netbird-e3/freeze/20260815-relabeled/artifacts/b/e3-phys-preflight-b-signed.hap \
...
- **当前 AUTH 固定 `attempt=initial`（retry N/A）**：`-TargetBindingConfirm` 与消费本 AUTH confirmation 的 `ready` Live/DryRun 均被 runner 强制 exact candidate pair + `attempt=initial` + `retry.basis`/`infrastructure_reason=N/A`——因此 **现有 generic retry 分支（`attempt: infrastructure-blocked-retry-1`）不进入本次路径**。**任一 gate 失败即停止，不重试、不换 ID**。任何后续尝试必须 **新治理**：先取得新的路线决策并重新登记新授权；当前 AUTH `AUTH-E3-PHYS1API26-20260815-0003` 不可用于任何 retry。
- **0002 被误判 Live 不构成 retry 依据**：其 sealed invalid evidence（`is_evidence=true`）虽满足「prior_record 为 Live evidence」的形式条件，但其成因为 **设备屏幕状态/环境问题**（S2 layout-checkpoint-scenario-2-entry-a-layout-mismatch：capture 布局树为锁屏页 `ScreenLockRootComponent`，`expected-bundle`/`start-control`/`stop-control` 缺失，设备屏幕锁定；已通过 Live 前解锁设备并保持屏幕常亮修复），不命中 retry 白名单 `hdc-usb-interruption` / `collection-storage-failure` / `runner-host-failure` 中任何一项；本 AUTH 已按新治理新建 pair 并以 `attempt=initial` 全新开始。旧 pair（`E3-PHYS-PREFLIGHT-20260815-0002` / `EV-E3-PHYS1API26-20260815-0002`）**已消费，不得复用**。
- **20260815-0001 / 20260814-0001 / 20260813 / 20260810 各 pair 同样已消费、不得复用**：`E3-PHYS-PREFLIGHT-20260815-0001` / `EV-E3-PHYS1API26-20260815-0001`（0001 S2 前台状态 invalid live 证据占用）与 `E3-PHYS-PREFLIGHT-20260814-0001` / `EV-E3-PHYS1API26-20260814-0001`（0001 blocked live 证据占用）及 `E3-PHYS-PREFLIGHT-20260810-0001` / `EV-E3-PHYS1API26-20260810-0001`（20260813 被中断 Live 的 sealed blocked 证据占用）及更早的 `20260808` 等 pair 一律不得复用。
...
- 本授权不豁免任何既有纪律：build drift、operator-aborted、功能 fail、scenario invalid、integrity violation、非基础设施 blocked 均不授权 retry；`consumed-blocked` / `superseded-unexecuted` / `INVALID-TIMELINE` 历史 ID 一律不得复用（含旧 pair `E3-PHYS-PREFLIGHT-20260815-0002` / `EV-E3-PHYS1API26-20260815-0002`、`E3-PHYS-PREFLIGHT-20260815-0001` / `EV-E3-PHYS1API26-20260815-0001`、`E3-PHYS-PREFLIGHT-20260814-0002` / `EV-E3-PHYS1API26-20260814-0002`、`E3-PHYS-PREFLIGHT-20260814-0001` / `EV-E3-PHYS1API26-20260814-0001`、`E3-PHYS-PREFLIGHT-20260810-0001` / `EV-E3-PHYS1API26-20260810-0001` 与更早的 `20260808` 等 pair）。
...
5. **禁止改写历史**：0002 及更早授权/证据登记记录保留不改写；旧 pair（`20260815-0002`/`20260815-0001`/`20260814-0002`/`20260814-0001`/`20260810`/`20260808` 等）不得复用；`$HOME/harmonyos-signing/netbird-e3/evidence-live-20260815-0002`（0002 sealed invalid 证据）**保留不改写**，其正式 evidence 登记由 campaign 后独立流程处理；本登记任务本身禁止提交/推送。
...
- 历史记录全部保留不改写：API23 initial（`EV-E3-PHYS1API23-20260806-0001`）、rebind（`EV-E3-PHYS1REBIND7-20260806-0001`）、build 确认（`EV-E3-PHYS1BUILD7-20260806-0001`）、API26 0001（`EV-E3-PHYS1API26-20260807-0001`，`consumed-blocked`）、API26 0002（`EV-E3-PHYS1API26-20260807-0002`，`consumed-blocked`）、host remediation（`EV-E3-PHYS1HOST-20260808-0001`）、外部 sealed blocked evidence `EV-E3-PHYS1API26-20260808-0001`（占用 20260808 pair）、`ADJ-20260808-0001/0002/0003` 登记、旧授权 `AUTH-E3-PHYS1API26-20260810-0001`（superseded，consumed）、`AUTH-E3-PHYS1API26-20260810-0002`、`AUTH-E3-PHYS1API26-20260813-0001`、`AUTH-E3-PHYS1API26-20260814-0001`、`AUTH-E3-PHYS1API26-20260814-0002`、`AUTH-E3-PHYS1API26-20260815-0001` 与 `AUTH-E3-PHYS1API26-20260815-0002`（历史登记保留不改写）；**0002 事故证据**：`$HOME/harmonyos-signing/netbird-e3/evidence-live-20260815-0002`（sealed invalid，`is_evidence=true`、`overall=invalid`、cleanup `verified-clean`、seal 完整、exit 2）占用 0002 pair，**保留不改写、不作为 retry 依据**；正式 evidence 登记（reviewed）由 campaign 后独立流程处理。

## 用户批准记录

本文起草即获用户批准（2026-08-15 下午，用户回复"准备好了"）。批准前本登记不构成可执行授权；批准后 `authorization_status: granted`，本登记是当前唯一生效的授权登记。

| 字段 | 值 |
| --- | --- |
| 批准时间 | `2026-08-15T13:1x:xx+08:00`（Asia/Shanghai；约 13:10，落盘精确值以生成时刻为准） |
| 批准方式 | 聊天确认（用户回复"准备好了"） |
| 批准后 authorization_status | `granted` |

## 执行注记（2026-08-15 下午）

1. **0002 Live 事实定性**：S1 pass；S2 invalid（`layout-checkpoint-scenario-2-entry-a-layout-mismatch`：capture 布局树为锁屏页 `ScreenLockRootComponent`，`expected-bundle`/`start-control`/`stop-control` 缺失，设备屏幕锁定）→ `SCENARIO_INVALID` → seal invalid（exit 2）；S3–S7 not-run-due-to-invalid；cleanup `verified-clean`。**根因为设备屏幕状态/环境问题，非冻结输入缺陷**。
2. **本次变更**：仅治理常量迁移（runner / selftest / freeze example 三文件 `20260815-0002` → `20260815-0003`：AUTH_ID / 候选 pair / 正向 fixture / authorization_id；OLD/负向 mutant 保留历史 ID 语义）；无 HAP/显示名/audit 脚本变更（0001 变更全部原样复用）。
3. **常量迁移与重新冻结**：runner/selftest/freeze example 治理常量迁移至 20260815-0003 系列，重新冻结（runner `40db928f…`、selftest `8b1fd2be…`、freeze example `72ae686d…`）；三文件已随本登记提交，`code_sha` 以 blocked freeze 绑定的最终 commit 为准（权威值由 freeze 承载）。
4. **修复措施（新增）**：Live 前操作员解锁设备并保持屏幕常亮（本 AUTH 强制生效）；`PYTHONUNBUFFERED=1`、证据目录轮询、`capture-pane` 转发沿用。
5. **按纪律停止**：0002 invalid 后不重试、不换 ID；本授权按新治理新建 pair 并以 `attempt=initial` 全新开始。E3 未关闭，E8 保持 `CLOSED`。
