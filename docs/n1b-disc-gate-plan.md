...
(3) **发布边界 = worker 终态原子标志置位（release）**：标志置位前快照字段对主线程**不可读**——join-timeout∧标志未置 → 主线程**无视快照**（快照可能半写）；`dw_return_class` 落值由 **R 前件分岔（r20，第二十轮 B-2——原「一律 (d)」把第三格整格截走、盒内置位成功路 F8(2) 烧 ID）**：`DW_RETURN` 缺（capture 为准）→ (d)；`DW_RETURN` 在 → **迟到竞态窗**（1000 ms 盒，见下方收口）；主线程读快照（acquire）只在 P12、且仅当标志已置。r18 冻结：标志是唯一可读门——消除松散发布读到初始化值的窗口，及「发射前写入」与「RETURN 缺则无 raw」的矛盾；
...
**迟到竞态窗收口（r19，主会话——sol 第十九轮中断前报告的格：JT=1 ∧ 标志未置 ∧ capture 已有 RETURN；r20 入口改述，sol B-02——P12 分支输入改纯本地可观测量：原入口「capture 已有 RETURN」与 (d) 第四前件「R 缺以 capture 为准」对 P12 不可判定（设备侧无 host capture 通道），capture 签名改由 runner sticky 例外行使）**：worker 冻结序为 写快照→RETURN→EXIT→置标志——RETURN 入 capture 与标志置位之间有真实窗口。
**P12 侧入口（r20）：JT 已登记 ∧ 终态标志未置 ∧ 快照写入完成信号已见（(d) 本地证据链不成立）即进 1000 ms 本地盒（不判 R）**；盒到期 P12 落 `flag-race-window-expired`（本地可判），runner 侧 sticky 例外按 capture 签名重建同 cause（R 的判定完全交 runner F8(2)；原句「capture 有 RETURN 即不入 (d)」随入口本地化废除——r20）；
该格下 P12 读快照须等标志（标志未置 → 快照门禁读）→ **主线程有界等待标志置位，冻结局部上界 = 1000 ms**（r19 定稿值；沿 D-W 节冻结轮询原语 = 原子标志 + 10 ms `clock_nanosleep`、非忙等；依据（r20 更正——原「EXIT 与置标志相邻」偷换前提：格 3 触发是 R=1 不是 EXIT=1）：**R 已入 capture ⇒ worker 剩余工作至多 = emit EXIT（一次 HiLog 发射）+ 置标志**——两步均无协议等待点；即使 HiLog 发射可阻塞（背压），超盒由盒到期支 graceful 收口（complete pass 非 fail），不构成 fail 面）。
...
枚举与 `dw_return_class` 域闭合（r14 改述；r17 计数更新：20 值按步分布——(0) 承接 skip 2 + 死亡收口 3 + `poll-never-returned` 1 + `flag-race-window-expired` 1（r20）、(1) 承接类 0/0b、(3) 承接普通 11 类含 true/false 两判定类，步序穷尽全部 20 值与「`_C` 在无 resolve」这一非 class 维度，无遗漏、无交叠、不留兜底桶）：判定表 13 类 + skip 编码 2 值 + pre-only 死亡收口编码 3 值 + r17 `poll-never-returned` 1 值 = **20 值**（r20 扩 flag-race-window-expired）；
