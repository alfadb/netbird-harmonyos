...
>
> **r19 修订（第十九轮审查修复：1 blocker / 4 major / 4 minor 处置）**：r18 经三席跨厂商隔离独立审查（绑定 cf7b084）——**deepseek（16 格机械枚举）pass 0B/4M**、**grok fail 1B/3M/4m**、**sol fail（连接中断后重派；中断前已确认与 grok 同根的竞态格）**；r18 两条 B 的修复全部确认落地（grok：「原 B-01 唤醒分叉不可达」「发布窗封闭」「第五形态有落点」）。去重 1 blocker（轨迹 12→8→4→3→5→3→2→1）——**标志门边界竞态，三格同根**（标志在 worker 序末尾、与 RETURN 入 capture 非同一事件两面）。
> r19 修订内容（两包）：
> - **W1 竞态收口**（grok 四点 + 主会话 cutoff 补充）：(d) 四前件→**五前件**（+终态标志未置——封死 (d) 截胡 (e) 的 fail-open）；`dw_join_result` runner 重建规则写死（D6b skip 在 → join-timeout，EXIT/RETURN 不得改写 joined——封死 F8(2) 烧合法 complete）；合法联合形态声明（JT∧13 类∧EXIT 同现 = 合法）；迟到竞态窗收口（第三格：capture 有 RETURN 即不入 (d)；P12 有界等待 **1000 ms 局部盒**；盒内置位 → 13 类 complete pass；
...
>
> **r23 修订（第二十三轮审查修复：1 blocker / 4 major / 4 minor 处置；sol 席三次连接失败、其终稿由主会话接手判定[局限声明在案、第二十四轮须复核]）**：r22 经三席跨厂商隔离独立审查（绑定 cfda2c1）——**deepseek pass 0B/4M**（cut-state 16 格穷尽——class 轴）、**grok fail 1B/4M/4m**（**T* 兄弟轴**）、sol 预警 2 与 grok 双向收敛（主会话判 B）。
> r22 的 5B 修复全部确认落地（发射序/方向 1/方向 2/:713 残止/F-SIG 互置/「RETURN 在」/内存序）；TOCTOU「class 轴是、全轴否」。去重 1 blocker（轨迹 12→8→4→3→5→3→2→1→2→3→5→1——**同族第五次：点时观测 vs 终态 capture 的比较 r22 只在 class 一轴关闭**；deepseek 16 格 pass 票恰好只走 class 轴——单轴 vs 全轴教训与 r16「同一钉两处落盘」同型）。
> r23 修订内容（两趟均执行层）：
...
>
> **r25 修订（第二十五轮审查修复：1 blocker / 2 major / 2 minor 处置）**：r24 经三席跨厂商隔离独立审查（绑定 d1ae06d）——**sol fail 1B/0M/0m、deepseek fail 1B/1M、grok fail 1B/2M/2m**；r24 修复（四处例外指针/闭表/T† 钉/头句首句改述）runner 侧全部确认落地——T\*/T† **runner 单值**。
> 去重 1 blocker（轨迹 12→8→4→3→5→3→2→1→2→3→5→1→2→1——**三席三视角完全收敛于同一格**：sol 主表 ④ 无 carve-out / deepseek「同上」桥接误扩 / grok **C_wd 构造**[watchdog 求值时点未绑到 cut 那一次 F load——A12 三输出不含 watchdog 留下 ②③ 间另读 FLAG 写 ④ 的合规源码洞]）。
> r25 修订内容（两趟均执行层）：
> - **Y1 watchdog 写侧时点绑定**（B-1 三席同向）：(d)/flag-race 落值清单显式 `dw_watchdog_killed`=⑤——F=0 时 P12 不求值 ①-⑤ 表、直接赋 ⑤（**cut-imputed unobservable**——sol 定名：由 cut=false 政策推定、非 EXIT 缺失观测）；①-⑤ 表加适用域句（JT=0/cut=true/pre-only 用本表；JT=1∧cut=false 不走——④ 前件是 capture 谓词、P12 侧不适用、不得另读 FLAG 代理）；闭表行 1/2 措辞改 cut-imputed（删「同上」——不以「EXIT 未见」为据）；
...
5. **终态 marker 双 marker 结构（r4 第一趟 R1，取代单一 `N1BDISC_RESULT`）**：终态结果通道拆为两个 marker，字段集分别逐字冻结（缺任一冻结字段 → `fail`，沿 MJ-7）——
   ...
   - **`N1BDISC_POST`**：在全序 **P12** 位点发射（P11 fd 清理之后）。字段集（逐字冻结）：`N1BDISC_POST|d6_items=<D6S1..D6S7 七子项逐字 ret/errno/reuse 结果全列>|dw_outcome=<D-W 结局逐字：poll 返回类（`dw_return_class`）、join 结果（`dw_join_result`）、watchdog 判定（`dw_*` 终态字段）全列>|ledger_digest=<fd-ledger **最终** digest，完整 64 hex（含 post 段增量）>|worker_terminal_at_p12=<true/false>`。
     （r16：`dw_return_class` 由探针主线程 P12 发射前唯一派生写入，runner 校验——见 D-W worker 序列段 r16 所有权裁定；r15 的 runner 派生声明随死锁解环改述。**r22 cut 记录**：新增字段 `worker_terminal_at_p12`（置于字段集末位、既有字段位次不变）= P12 派生 class 时终态标志 `dw_worker_terminal` 的读值——runner 对 JT=1 格的 cut-aware 校验见分域声明区 cut-state 规则（r22，sol B-01 TOCTOU 闭合）；complete 形态必载、pre-only 形态无 POST 自然不载）
...
`dw_return_class` 落值由 **本地前件分岔（r21 改述 r20 分岔（第二十轮 B-2）——R 前件的 capture 视角描述废止、与下方 (d) 本地证据链与竞态窗「P12 侧入口」段统一（r23：行号锚改节内锚），P12 无 capture 通道）**：快照写入完成信号未置 → (d)；信号已见 → **迟到竞态窗**（1000 ms 盒，见下方收口；capture 视角的 R 判定由 runner 侧行使）；主线程读快照（seq_cst store/load——与信号规格统一，r22 更正 grok M-04：原 release/acquire 措辞为内存序双规格残留）只在 P12、且仅当标志已置。
r18 冻结：标志是唯一可读门——消除松散发布读到初始化值的窗口，及「发射前写入」与「RETURN 缺则无 raw」的矛盾；
`dw_outcome` 的**全部**派生字段（`dw_return_class`/`dw_join_result`/`dw_destroy_distinguishable_from_timeout` 及 `dw_*` 终态字段）同此所有权——探针 P12 写入、runner 重建比对、不一致挂 F8(2)（r17 扩展：原仅 class 声明，其余字段同批）**（r24 例外指针，第二十四轮 B-1 三席收敛——JT=1∧`worker_terminal_at_p12`=false 格的 `dw_return_class`/`dw_poll_*` 四字段/`dw_watchdog_killed` 不适用本句重建比对，见 cut-state 规则 (B) 全轴扩展）**；
...
（**去循环（r21）**：runner 读 RACEWIN marker 存在性、不读 POST cause——P12 错写 class 时 RACEWIN 在/不在与 class 无关，错写可被 F8(2) 独立抓住；「同一事实两算」的前提在该格不成立——P12 输入面=标志门快照[禁读]、runner 输入面=capture 签名（D6b skip ∧ RACEWIN），两函数不同源、逐字段比对必然不一致——r20 写死该格重建签名使两侧落同一 cause）；
**（r22：该 sticky 现为 cut-state 规则 (B)(i) 的分支一致性实现——JT=1∧cut=false∧RACEWIN 在 → runner 重建 flag-race；RACEWIN 在 ∧ JT=0 或 cut=true → capture 自相矛盾 → F8(2)[deepseek M-02 的 spurious RACEWIN 矛盾面]；签名 `D6b skip ∧ RACEWIN` 保持不变）**
...
**信号规格（r21 冻结——grok M-01）**：快照写入完成信号标识符 `dw_snapshot_written`（进程内原子布尔、初值 false、置位后不自旋不重置）——可见性 = seq_cst store（worker 侧）+ seq_cst load（P12 侧每次读：盒入口一读定进盒、盒到期再读一读**复核 SIG 单调性（违反 → F8(3)）**）；终态标志同批规格化：标识符 `dw_worker_terminal`、同规格（seq_cst store/load、初值 false、置位后不重置——原 release/acquire 仅为上方 worker 序列「(3) 发布边界」段的序边描述（r23：行号锚改节内锚）、无标识符与内存序规格）。
runner 从 capture raw **独立重建并比对**，不一致 → **fail，挂 F8(2)**（capture 内自相矛盾：探针 P12 派生值与 runner 从 capture raw 的独立重建值不一致——同一事实两算不符，记录器完整性缺陷；r17 挂载，grok B-03 = sol M-01 两席收敛：原『沿 E2 模式』未入闭集字面，按局部句 fail、按闭集实现漏判 = fail-open），重建值不作为第二事实源、只作校验（r24：cut=false 格例外见 cut-state (B)——class/poll raw/watchdog 透传校验）；
...
→ emit `N1BDISC_DW_EXIT` → 置 worker 终态原子标志（**r18 序对调：原「置标志 → emit EXIT」废除**——EXIT 发射纳入标志门，标志语义 = 「worker 全部输出已完成」（raw 已写**且** EXIT 已发射），sol M-01 消 P12 的 EXIT 小窗；r20 序补：本序「写快照」与 emit `DW_RETURN` 之间新增**置快照写入完成信号**原子（写完六字段后即时置位、先于 RETURN 发射——(d) 本地证据链与竞态窗入口的分流门，见上方「EXIT 存在性无小窗」段与分域声明 r20（r23：行号锚改节内锚））→ 线程返回。
...
**分域声明（r16，sol B-02；r17 重写为四子域，三席收敛，第十七轮 B-01；r18 扩为五子域 (a)-(e)；r19 补迟到竞态窗收口）**：`dw_return_class` 的**七步求值序（(a)→(c)→(b)→(d)→(e)→竞态窗→13 类域——r20 更正标题：五子域为子域列举、求值序含收口步）**——
**求值序（r19，deepseek M-02）：(a) → (c) → (b) → (d) → (e) → 迟到竞态窗收口 → 兜底 13 类域**——先到先得、命中即终止。
...
**(b) pre-only 死亡收口编码**（r19 补限定）：进程死亡、判定输入不可得**且无 `SKIP|item=destroy`**（有该 SKIP 则输入可得、先由 (c) 承接）→ 按 `destroy_call_state` 五态分流落 `destroy-not-reached`/`post-destroy-unobservable`/`call-boundary-incomplete` 三透传值，不进 13 类；
**(c) 类 0（`destroy-skip-proven`）——不要求 RETURN（r17 新增支）**：判定输入 = `N1BDISC_SKIP|item=destroy` marker 存在性（机器可判、与 poll 无关），有该 SKIP 即类 0、无论 RETURN 在否——其真实可生成路径 = barrier-never-observed 顺延（skip 表 destroy 行的 SKIP 与 D-W skip 同现、被 (a) 先行全量指派截走，不入类 0——r19 更正 deepseek M-02）（grok 原话：类 0 仅认 item=destroy SKIP，不要求 RETURN）；
...
**(d) complete∧join-timeout 具名收口（r17 新具名 cause；r18 前件改述；r19 扩为五前件）**：`unobservable(cause=poll-never-returned)`——触发五前件（r19，grok B-01——原四前件使 (d) 可截胡 (e)）= 非 skip（无任何 `SKIP|item=destroy`）∧ 进程活到 P12 ∧ P10 终态轮询盒到期（`join-timeout` 已登记、`join-timeout-worker-abandoned=true`；
...
快照写入完成信号为 worker 侧本地量（写快照后即时置位、先于 RETURN 发射）。（**r23 废止点时全称**——原「其未置 ⟹ RETURN 必未发射 ⟹ capture 无 RETURN」为点时读对终态的非法全称，已被 cut-state (B)(iii)/全轴扩展取代：S_cut=0 后 worker 仍可在决策与 POST 之间完成全序、其 RETURN 入 capture 是合法迟到完成非矛盾——r21 在竞态窗侧 :724 区更正过同款蕴含、本句在 (d) 支内未同步、r23 补正。）r18：标志未置是本支机器门，原 r17 括注「终态标志只在 RETURN 后置位 → 该路径必然无 RETURN」随快照发布边界重写废除；
**r18 归因洁净补注（D6b 重裁侧，grok B-01 = sol B-02）**：进程内 marker 发射与 capture 落盘是两个事件，（**r23 废止**——原「迟到 RETURN 入 capture 则本前件失败」以活规则语气陈述已被 (B)(iii) 取代的论证：(d) 的 P12 侧判定只依赖本地证据链[五前件]、迟到 RETURN 的 capture 存在性不再是任何前件的否定项；
归因洁净部分保留：D6b skip 保证主线程在 P12 前不触碰 fd_dup、晚到 RETURN 不被唤醒或抑制——**归因洁净由 skip 保证，不由 close 的平台行为决定**）；D6b skip（D6 节 D6b 段 r18 重裁）保证主线程在 P12 前不再触碰 `fd_dup`，晚到 RETURN 若发生不会被 D6b 的 close/read 唤醒或抑制——**归因洁净由 skip 保证，不由 close 的平台行为决定**）
...
(iii) 迟到的 RETURN/EXIT 在 capture **不构成矛盾**（宁缺勿误——P12 的 cut 记录是 class 的事实源，runner 只做分支一致性）。
**(C) JT=0 格不受本规则影响**（无竞态族——正常重建比对）。
合法域门通过后，方进入下述派生序（r15 改述：原「13 类判定表」的自上而下匹配现按上述前置门序执行；类 0/0b 已前置、行 1-11 为终末匹配）。**首个命中即定类并终止匹配**：

  | 优先级 | 类 | 判定条件（命中即终止匹配，其余维度不再看） |
  | --- | --- | --- |
  | 0 | `destroy-skip-proven`（r9 改名改条件，原 `destroy-never-called` 废除——缺 `_C` ≠ 未调用，见「`destroy_call_state` 五态表」） | 协议显式发出 `N1BDISC_SKIP\|item=destroy`（即五态 `not-called`，**唯一可证「未调用」的途径**）——**不归因、不解锁路径①**（现行路径中该 SKIP 产生于 D-W 整体被 skip 的分支与 r9 第五步 barrier-never-observed 顺延路径两处，本行保证该组合在表内仍有唯一落点）（r19 注：no-live-connection 路径经 (a) 先行指派实际不入类 0，此处保留为 SKIP 字面溯源清单——判定路由以分域求值序为准） |
...
`dw_inwait_samples`、`dw_poll_return_elapsed_ms`、`dw_poll_ret`、`dw_poll_errno`、`dw_poll_revents`（以上 poll raw 四字段 r17：complete∧join-timeout 形态各记 `unobservable(cause=poll-never-returned)`——poll 未返回、RETURN raw 无值，见分域声明 (d)；r21 补（grok M-03）：迟到竞态窗到期形态各记 `unobservable(cause=flag-race-window-expired)`——标志未置、快照门禁读、P12 侧 raw 不可得，见竞态窗收口段；`dw_drain_*` 不盖、保 DRAIN marker 真实值；
...
含未知位的值由未知位前置门（r9 第五步 BL-3 升格，见本节「`revents` 编码冻结」段；r16：**仅 0/0b 前置检查未命中的输入**到达该门即定类 `other-revents`，无 SKIP 且 `_C` 缺的未知位输入已归 0b）直接定类 `other-revents`、不进入表内匹配（原值十进制逐字入档），不 fail——与 `N1BDISC_DW_RETURN` marker 的 `revents=<十进制 bitmask 单值>` 同一编码）、
  `dw_return_class`（判定表 13 类 + r9 第五步 BL-5 纳入的 skip 编码 2 值 `unobservable(cause=no-live-fd)`/`unobservable(cause=dup-failed)` + r10 纳入、r12 拆分、r13 第三包再拆分的 pre-only 死亡收口编码 3 值 + r17 新增 complete∧join-timeout 收口编码 1 值 `unobservable(cause=poll-never-returned)`（分域声明 (d) 支）+ r20 竞态窗收口编码 1 值 `unobservable(cause=flag-race-window-expired)`（迟到竞态窗收口支）= **20 值**；
...
r12 的逐值具名 cause 冻结沿袭不变（blocker 7，sol B-07——原裸 `unobservable` 不带 cause，违反 r9 字面拼法冻结的 `cause=` 逐字比对规则，complete 主线（destroy resolve + `pre-destroy-ready`）落「其余」时 POST 无合法单值））**：
    - **四步有序互斥（r14，blocker 1，grok B-01 = sol B-03 两席收敛——原 r13 二维声明下，「有 `DW_RETURN`、无 `_C`、无 resolve」的合法平台死亡两维均不命中、派生字段无落点 → F8 烧掉 PRE 后合法平台终止；「已发起而无 resolve 证据」措辞又在 E3 方向（`_C` 在、无 RETURN、无 resolve）与透传行两读。求值序先到先得、命中即终止，每步接住前步未命中的输入）**：
      - **(0) 直接透传**——`dw_return_class` ∈ {skip 编码 2 值 ∪ pre-only 死亡收口 3 值 ∪ r17 complete∧join-timeout 收口 1 值 `unobservable(cause=poll-never-returned)` ∪ r20 迟到竞态窗收口 1 值 `unobservable(cause=flag-race-window-expired)`} → 输出逐字等于输入编码本身（destroy 未被调用/未达、判定输入不可得、或 worker 未返回致 resolve 两维前提不成立——r17 新值沿 skip/死亡收口同款理由，resolve 与否无从谈起）；
      - **(1) 类 0/0b 直接映射**——`destroy-skip-proven` → cause 逐字沿实际发出的 `N1BDISC_SKIP|item=destroy` 位点字面（预注册 `no-live-connection`/`barrier-never-observed`，与 V2 (1b) 清单同一；r19 注：no-live-connection 路径经 (a) 先行指派实际不入类 0，此处保留为 SKIP 字面溯源清单——判定路由以分域求值序为准）；
      `destroy-call-unobserved` → `unobservable(cause=destroy-call-unobserved)`——**两类不入任何 resolve 分区**：`_C` 缺、resolve 证据不可得是这两类路径的常态而非异常，类 0b 的输入恰恰要求「有 RETURN、无 SKIP、无 `_C`」（判定表优先级 0b），把它收进「仅 resolved」的第二维正是原洞的成因（历史沿革句——现行口径为四步，r17 注）；
      ...
      **对齐注（防混淆）**：类 0b `destroy-call-unobserved`（判定表类：有 RETURN、无 SKIP、无 `_C`）与死亡收口 cause `call-boundary-incomplete`（五态 `_T` 在 `_C` 缺、判定输入不可得）是 20 值域中**两个不同的值**（r17 计数同步），输入域不同，本表内前者走 (1)、后者走 (0)，无交叠。
...
枚举与 `dw_return_class` 域闭合（r14 改述；r17 计数更新：20 值按步分布——(0) 承接 skip 2 + 死亡收口 3 + `poll-never-returned` 1 + `flag-race-window-expired` 1（r20）、(1) 承接类 0/0b、(3) 承接普通 11 类含 true/false 两判定类，步序穷尽全部 20 值与「`_C` 在无 resolve」这一非 class 维度，无遗漏、无交叠、不留兜底桶）：判定表 13 类 + skip 编码 2 值 + pre-only 死亡收口编码 3 值 + r17 `poll-never-returned` 1 值 + r20 `flag-race-window-expired` 1 值 = **20 值**（r21 加法更正 grok m——原式未含 r20 扩值）；
...
五态 `call-boundary-incomplete` → 逐字透传 `unobservable(cause=call-boundary-incomplete)`（**r13 第三包，sol M-01：五态定义明言该态「既可能尚未调用，也可能已进入调用」，「destroy 已过」对它不成立——r12 把它与 `call-returned` 同记 post-destroy 是事实错误，改沿 `dw_watchdog_killed` r13 有序互斥表 ② 同款透传自身 cause**）。
    - **(步 0) complete∧join-timeout 收口编码 1 值**（r17 新增；**r18 注：分域 (e)「标志已置∧capture 无 RETURN」为 fail(F8(2)) 收口、不赋 class 值，不入本表——本表只收可赋值形态**）——
`unobservable(cause=poll-never-returned)` → 本字段输出逐字等于该输入编码本身（worker 未返回、`_C`-resolve 两维前提均不成立，沿 skip/死亡收口同款理由；非 skip、进程活到 P12、`join-timeout` 已登记、（r23：本步按 class 值透传——poll-never/flag-race 的产生不要求 capture 无 RETURN[cut-aware]，括注的原互斥表述废除）、终态标志未置五前件见分域声明 (d)（r19 扩为五前件；r20 第四前件改本地证据链——见 (d)））。
    - **(步 0) 迟到竞态窗收口编码 1 值**（r20 新增）——`unobservable(cause=flag-race-window-expired)` → 透传（两维前提不成立——P12 侧无 raw、runner 侧按 sticky 例外重建同 cause）。
...
r10：pre-only 死亡收口编码（r12 拆分为 `unobservable(cause=post-destroy-unobservable)` 与 `unobservable(cause=destroy-not-reached)` 两值；r13 第三包再拆出 `unobservable(cause=call-boundary-incomplete)` 透传值，合计三值）同不解锁——该组值仅当判定输入不可得时登记，`DW_RETURN` marker 不在（条件 6 不成立）、`fd-event-like`（条件 2）不可得，无路径①/②判定面；
r17：`unobservable(cause=poll-never-returned)` 同不解锁——（r23：poll-never 的产生路径含迟到完成竞态格[cut=false]，RETURN 的 capture 存在性不与其互斥——路径① 条件 2 要求 `fd-event-like` 本就不含 poll-never，本理由句的互斥表述废除）、无 poll 返回输入（条件 2/3/4 均不可得），无路径①/②判定面；
r21（grok M-03 域扩传播）：`unobservable(cause=flag-race-window-expired)` 同不解锁——类非 `fd-event-like`（条件 2 不成立；R=0 子窗 `DW_RETURN` 不在 capture、条件 6 亦不成立），无路径①/②判定面）。
...
**r19 第三路径收编（grok M-01）**：join-timeout-abandoned（D6b skip）——本表原两列覆盖 `no-live-fd`/`dup-failed` 矩阵终局分支；join-timeout 分支的 skip 语义不同（VPN live、destroy 已执行、仅 worker 未达终态），其 skip 指派预注册于 D6 节 D6b 段 r18 重裁段与 P10 行（`SKIP|item=D6b|cause=join-timeout-abandoned`；
`u4_dup_*` 落 `unobservable(cause=d6b-skipped-join-timeout)`、POST `d6_items` D6S4..S7 落 `skipped(cause=join-timeout-abandoned)`）——**两套 cause 字面为同一事件在 unobservable/skipped 两编码族的双字面，非笔误**（r19 交叉引用句，grok M-04 = deepseek M-04）。
...
P11 探针自有 fd 清理（d4_send_socket/d5_sink_socket/d6b_reuse_probe_socket 逐个 close 并以 F_GETFD 复核 EBADF；未创建项登记 not-created）（仅 d4_send_socket/d5_sink_socket/d6b_reuse_probe_socket 三项，不含 `fd_dup`——r19）
...
| P12 迟到竞态窗等待 | 1000 ms（r20 补行——grok M-02：时间盒表核对是 A4/gate 3 静态面，缺行即缺口） | 盒到期 → flag-race-window-expired 收口 |
| D6 各步 | 即返 syscall | — |
...
| A4 | **除第二趟修订集群 A5(b) 已登记的 `pthread_join`（worker 终态原子标志置位后调用；该调用无用户态超时出口——r3 第一趟 D5 显式豁免，freeze 静态审查不得因 join 缺超时出口判 A4 不通过）外**，全部等待点经单调时钟且带本文时间盒；其余零无界阻塞调用（零 `pthread_timedjoin_np`、零非清单 sleep/等待原语）。**P10 行与观测窗兜底的关系（冻结）**——外提为表后「A4 注」（r4 第四趟 W1 重构，内容逐字不变）；**P1 `dlopen`/`dlsym` 不属本断言「等待点」——本协议等待原语闭集 = `poll` 与 `clock_nanosleep`（探针 syscall 面节），dlopen/dlsym 为工作调用非等待原语，其 10 s 按时间盒表 P1 行为测量/分类参数，freeze 静态审查不得因 D1 实现无 deadline 分支判 A4 不通过** |
...
- `D8_MTU`/`D8_STORM_BEGIN`/`D8_STORM_END`；`D7_BEGIN`/`D7_END`；`DW_SPAWN`/`DW_DRAIN`/`DW_BARRIER`/`DW_INWAIT`/`DW_DESTROY_T`/`DW_DESTROY_C`（r7 X4 新增 post-invocation marker）/`DW_RETURN`/`DW_EXIT`/`DW_RACEWIN`（r21 新增：迟到竞态窗盒到期 raw marker）；`D6S1_B`..`D6S7_B`/`D6S1_R`..`D6S7_R`；
...
- **F8** 解析器 / 真值表缺口（= criteria-gap 判别方法 **(2)(3)**——r10 论域切分，A m-03：F4 限判别方法 (4)，本条不再与 F4 的「(2)-(4)」重叠——(2) 输入在 capture 内自相矛盾（含探针 P12 派生字段与 runner 重建值不一致——r17）（r24：cut=false 格的迟到分叉字段例外——见 cut-state (B)；该格比对走透传校验，非重建比对）、(3) 派生规则/真值表未覆盖该输入组合——**记录器未尽职，不是死因**；**未知 `revents` 位不属本条**，归 `other-revents` 不 fail（r9 第五步升格为未知位前置门，沿 S2/④；0/0b 前置检查未命中的输入；无 SKIP 且 `_C` 缺的未知位输入归 0b，r16）；
...
**r20 计数更新（第二十轮 B-1）**：`dw_return_class` 再增迟到竞态窗收口 cause `unobservable(cause=flag-race-window-expired)` 1 值 → 判定表 13 类 + skip 2 + 死亡收口 3 + 1 + 1 = **20 值**（四步步 (0) 同步 6→7 值）；`dw_join_result` 仍不随扩（**10 值**不变——r20 sticky 例外只改 runner 重建落点、不加域值）；
...
(2)-(4) 为**记录器义务未尽（解析器/真值表缺口）**，命中任一即判 `fail` 并逐字保留 raw 原文——(2) **解析域缺口**：chunk 重组、marker 解析、字段提取等解析器无法处理已到达的原始数据，及 **capture 自相矛盾 / P12 派生与 runner 重建不一致**（r18 随 F8 同步扩入——同源两写 + 标志门下分叉只可能来自记录器缺陷，grok M-03）（r24：cut=false 格的迟到分叉字段例外——见 cut-state (B)；该格比对走透传校验，非重建比对）（解析器缺陷不是平台事实）；
...
**r9 第五步 BL-3 混合位用例，r16 同款两钉**：`revents=72`（未知位 64 + `POLLERR`）与 `revents=65`（未知位 64 + `POLLIN`）各拆两钉——`_C` 缺 → **0b**、`_C` 在 → 由未知位前置门定 `other-revents`、原值入档、不 fail、**不解锁路径①**（不得命中行 7/行 8））× elapsed 4500 边界 × drain 终态 × poll ret/errno 组合，见 D-W 节）；
   **r12 派生字段逐值落值用例（blocker 7，sol B-07 验收钉；r13 起改述（r15 起按四步口径：(0)/(1) 透传与直接映射、(2) destroy-unresolved、(3) 逐值表）；r16 拆写（sol M-01：原「18 个 class 在 resolved 前提下逐值过步 (3)」与「skip/death 走 (0)/(1)」同句两读，拆除）**：
   `dw_return_class` **20 值**（r17 补 `poll-never-returned` 1 值；r20 补 `flag-race-window-expired` 1 值；r13 第三包死亡收口再拆更新；当轮 17 值、原 16 值）按步分域逐值落值——步 (0) **7 值**（r17：skip 2 + 死亡收口 3 + `poll-never-returned` 1；r20 + `flag-race-window-expired` 1）+ 步 (1) 2 值逐值透传/直接映射；**普通 11 类**按 resolved（步 (3) 逐值表）/ 无 resolve（步 (2) destroy-unresolved）分核——
`fd-event-like`+10 s 内 resolve → `observed-true`、`timeout-like`+resolve → `observed-false`、普通 11 类的其余 9 值按 r12/r13 具名 cause 落 `unobservable(cause=…)`
（步 (0) 7 值逐值透传——skip 2 + 死亡收口 3 + `poll-never-returned` 1 + `flag-race-window-expired` 1（r20），与上句统一（r18：原「skip 5 值」为 r17 扩 `poll-never-returned` 前的旧计数残留，grok m-01 = sol m-01 两席收敛）、类 0/0b 2 值在步 (1) 直接映射，不入步 (2)/(3)——r16 计数随拆写同步：原「其余 16 值」混入了 (0)/(1) 域），逐格核对无裸 `unobservable`、无缺项
...
期望——`dw_return_class=unobservable(cause=poll-never-returned)`（r17 新值、20 值域内（r20 扩）；非 skip、进程活到 P12、join-timeout 已登记、RETURN 缺（r20 按本地证据链第四前件读——worker 挂死未写快照、本地链成立）、终态标志未置——五前件齐（r19；第五前件在本夹具恒真——worker 挂死未达置位序））、`dw_join_result=join-timeout`（**既有本体值——join 域不扩、10 值不变（r17 核对结论：该 cause 仅入 class 域）**）、`dw_destroy_distinguishable_from_timeout`=步 (0) 透传（输出逐字等于输入编码本身）；
`dw_poll_ret`/`dw_poll_errno`/`dw_poll_revents`/`dw_poll_return_elapsed_ms` 各记同 cause（poll 未返回、RETURN raw 无值），`dw_drain_*` 保 `DW_DRAIN` marker 真实值（不盖，r10 反 blanket 法理）→ POST 各字段有值、无字段缺项 → fail 闭集（F1-F6、F8、F9）均不命中 → verdict **`complete pass`**（终态标志只在 RETURN 后置位 → 该合法 complete 形态必然无 RETURN，不得烧 ID）。（r23：本句为 r17 夹具形态的点时描述——限「worker 挂死未写快照」的本钉形态，非活规则；迟到完成形态由 cut-aware 全轴承载，见分域声明 r23）
**r19 迟到返回 complete pass 钉（grok B-01 验收钉——分域 (d) 五前件 + `dw_join_result` 重建规则验收）**：夹具沿上方 join-timeout 夹具至 P10 盒到期（JT 登记 + `SKIP|item=D6b` + P11），此后 P12 前迟到 worker 完成序列入 capture（`DW_RETURN`/`DW_EXIT` 在、终态标志置位）→ class 走 13 类（快照经标志门可读）、`dw_join_result=join-timeout`（重建规则：D6b skip 在 → 不由 EXIT/RETURN 存在性改写 joined）、D6b skip 编码不变
   watchdog=④[r25 更正 P2 残留②：本钉形态 F=1→cut=true→(A) 走 ①-⑤ 表、④ 前件 EXIT 在（迟到完成含 EXIT）→ observed-false——⑤ 仅绑 F=0 格（cut-imputed），原插「⑤」对 F=1 误用]
   F8(2) 按重建规则一致 → **complete pass**（合法迟到完成不得烧 ID）。
...
**r19 第三格钉（迟到竞态窗收口验收——分域声明 r19 迟到竞态窗收口 + 1000 ms cutoff）**：夹具同 join-timeout 夹具至 JT 登记 + D6b skip，P12 到点时终态标志仍未置但 capture 已有 `DW_RETURN` → 主线程有界等待（1000 ms 局部盒、冻结轮询原语）：(a) 盒内标志置位 → 读快照走 13 类 + `dw_join_result=join-timeout`（重建规则一致、F8(2) 不命中）→ **complete pass**（r20 改述：P12 侧入口不判 R——本地信号已见即进竞态窗，见分域声明 r20 入口）；
   (b) 对照——**1000 ms 到期标志仍未置**（worker 在 EXIT 前**或 EXIT 后置标志前**（r23 更正 grok m-03：『release』为内存序旧词、活规格已 seq_cst）某点被永久抢占/阻塞（r20 补子窗——见盒到期支 flag-race cause 描述））→ `dw_return_class`/poll raw 字段落 `unobservable(cause=flag-race-window-expired)`、`dw_join_result=join-timeout` 不重写、D6b skip 编码不变、watchdog=⑤[r25]
   → POST 各字段有值 → 不命中 F4/F8/F1 → **complete pass**（合法竞态残余宁缺勿误；r19 定稿 cutoff——禁止无界等待未知事件、禁止拿外层 525 s 当本等待上界）。
   ...
   **r22 TOCTOU 钉**：JT=1 + P12 读 S=0/F=0 选 poll-never + 主线程 POST 前被抢占 + worker 完成（RETURN/EXIT 入 capture）+ 主线程恢复按已做决定写 POST（cut=false）→ runner cut-aware (B)(iii)：迟到 RETURN/EXIT 非矛盾 → **complete pass**（r22 核心验收——原 r21 该格 F8(2) 烧合法调度，sol B-01 决胜格）（r24：本钉原断言仅 class 轴——全轴验收见 r23 T\* 钉与 r24 T† 钉）；
   **r22 方向 1 钉**：cut=true（盒内置位）却发 RACEWIN + 写 flag-race → （r23 更正 grok m-01：cut=true 时 (B) 不适用——fail 由 (A) 重建 13 类≠flag-race + sticky「RACEWIN∧cut=true」矛盾面共同承载，原「(B)(i) 双违」误引） → **fail(F8(2))**；
   ...
   **r23 T* 全轴钉**（grok B-01 验收——决胜格全轴）：T* 构造（JT=1 + P12 读 S=0/F=0 走 (d) + 写 poll-never/cut=false/poll raw=unobservable/watchdog=⑤ + POST 前抢占 + worker 完成 RETURN/EXIT 入 capture + 主线程恢复写 POST）→ runner 全轴分支一致性：
class (B)(iii) ✓ + poll raw 透传校验（unobservable 与 class=poll-never 分支一致）✓ + watchdog 透传校验（⑤ ∈ 合法域且与 poll-never 分支一致——**迟到 EXIT 不参与校验**）✓ + join sticky ✓ → **complete pass**（r23 核心验收——原 r22 该格 watchdog/poll raw 轴 fail-closed）；
   **r24 T† 钉（grok B-2 验收——决策前已发射形态）**：JT=1 + SIG=1 + 盒到期 F=0 + worker 已 emit RETURN+EXIT 入 capture 而未置 FLAG（R=1∧EXIT=1∧F=0——EXIT 在 P12 决策前）→ P12 写 flag-race/cut=false/watchdog=⑤/poll raw=同 cause → runner 闭表校验：class=flag-race ⟹ watchdog 允许集恰一值 ⑤ ✓、poll raw 同 cause ✓——**EXIT 在 capture 不参与（决策前已发射同决策后迟到一律）** → complete pass（r24 核心验收——原 r23「迟到不参与」措辞下该格按 watchdog ①-⑤ 表走 ④ 烧 ID）；
   **r23 RACEWIN 行次钉**（grok M-02）：盒到期诚实发射的 capture 中 `N1BDISC_DW_RACEWIN` 行次必须先于 `N1BDISC_POST`（冻结序 ①③ 的 selftest 面——POST-first 实现必须被 selftest 抓住）；
   **r23 T' R=0 夹具钉（grok M-02 前半）**：SIG=1∧R=0 盒到期（本征窗）complete pass——capture 无 RETURN、RACEWIN 在、cut=false、class=flag-race → (B)(i) ⟺ + sticky 重建一致 → pass（r22 已验、此处补独立夹具钉死）。
...
**verdict 由完整性轴承载**——位点在短时步集 ⇒ `N1BDISC_PRE` 未发 ⇒ **fail（F2）**，且 marker 序列自相矛盾 ⇒ 叠加 **fail（F3）**；本用例验证的是第 3 支不稀释为 true、非 verdict 断言（原「→ 不 fail」随 r12 删除——该断言与 F2/F3 正文规则非单值，两席收敛确认）；
   ...
   - `protocol=complete` + 命中崩溃签名（r12 限定 Ⅰ/Ⅱ 夹具另发 `N1BDISC_POST`；原「Ⅰ/Ⅱ/Ⅲ 任一」废除——Ⅲ 的短时步位点与 POST 结构上不可达：POST 在则 `last_visible_site` 至少 P12（POST 为全序末位阶段 marker，P12 后无 `N1BDISC_` marker）（r22：P12 内 RACEWIN 若发射先于 POST——全序内 P12 位点的内部序，P12 后无其他 marker 的语义不变），第 3 支位点守卫必不满足，Ⅲ 的 complete 形态不存在，blocker 6）；
...
> **2026-09-06 判据变更 CC-2（首对 pair gate 5 元组漂移触发，用户授权重绑；登记续于文末——沿审查登记册文末追加先例，避免文首插入造成第三次行号锚位移；文首状态行指向本块）**：首对 pair（`AUTH-N1BDISC-PHYS1API26-20260905-0001`，授权登记 `docs/evidence/n1bdisc-authorization-2026-09-05-0001.md`）于 gate 5 三探针实测设备软件版本 `PLA-AL10 7.0.0.105(SP6C00E105R7P3)` ≠ 原冻结值 `PLA-AL10 7.0.0.102(SP8C00E102R7P3)`（设备于 2026-08-30 G0 实测 7.0.0.102 后 OTA 升级）——按本判据「目标绑定门」条款（`:1436`）裁 blocked record + 退役该 pair（blocked record：`~/harmonyos-signing/netbird-n1bdisc/records/target-binding-confirmation-20260906-0001.json`，SHA-256 `d2f59da0a66d9357e59fdcae47b390f3e8780e873a56f2a3267955f3665c7fbd`）。用户于 2026-09-06 会话内显式授权「重绑新元组继续」。变更面：`:268` 冻结元组软件版本字面（`7.0.0.102(SP8C00E102R7P3)` → `7.0.0.105(SP6C00E105R7P3)`）+ `:269` 版本核对警示补 CC-2 印证句 + `:1436` gate 5 历史差异例更新 + 本登记 + 状态行；**行号均按 CC-1 后现行编号（本块文末追加，`:1786` 以下零位移）**。**除设备侧元组外本判据的其余冻结面不变**：API 26 / aarch64 / arm64-v8a 不变——**CC-2 重审 m-2 登记**：此三者为「沿同设备历史实测沿用 + build 级 OTA 性质推断」，本次 gate 5 未复测（gate 5 白名单仅三探针、不得扩）；arch 为硬件属性不可经 OTA 改变，API level 绑定主版本 7.0.0 不变；与 rebind8「实测不从 build 推断」纪律的表面张力登记于案，不阻塞；SDK/sysroot 锚点（`:309` 起，26.0.0.821 实测对账 9/9 PASS，见 `docs/n1bdisc-sdk-anchor-reconciliation-20260905.md`）为 SDK 侧、与设备小版本无关；syscall 闭表、marker 字面集、时间盒表、MR 表、HDC 白名单、faultlogger 候选集与 13 门流程均不变。**新元组的设备侧行为依赖（watchdog / faultlogger 格式 / loader 接受性等）本判据原即以实测为目的、不预置结论**；Fault_Type 候选集仍未实测收敛（gate 3 条款原样适用，未实测依赖随新 pair 证据登记）。**本变更须跨厂商隔离重审通过后方可按新元组恢复可 freeze/测量状态；首对 pair 退役不因本变更撤销；新治理须新 AUTH/pair/evidence 三 ID 并从 gate 1 重走门序列**（实现资产与首对 freeze-1 符合性结论可被新治理引用）。
...
>
> **判据解释裁决（另一跨厂商隔离席）——实现不违反判据**：(1) 时间盒表是「位点预算/分类参数表」，P1 属参数行；`:1031` 表头为「位点」而非「等待点」，表内已有 `:1040` P5T「即返」、`:1049` D6「即返 syscall」等非等待行；(2) `:1034` P1 行的「超时分类」列不含任何探针侧盒到期动作（对照 `:1035`/`:1043`/`:1048` 均有）；(3) 该 10 在 `:1059`（预算加数 467 s）与 `:1231-1232`（位点分类参数）被非控制流消费；(4) `:1029` 的可执行面 A4 由闭表定义「等待点」（`:358` 等待原语仅 `poll` 与 `clock_nanosleep`），`dlopen`/`dlsym` 在 `:355` 以工作调用列入。**`:1029` 的单一豁免措辞（原文见本块 ① 改前对照）为事实上的过度断言**：P1 是第二处「无用户态超时出口、由观测窗收口」的位点却未登记；`:1434` 把「时间盒表核对」列为无机器检查器背书的 reviewer 独立义务，按字面执行必卡。**探针内实现真实 10 s 机制技术上不可能**：A1（`:1076`）禁第二线程；`alarm`/`timer_create`/`setitimer`/`sigaction` 不在 `:355-360` 闭表内（`:362` 明定超表 syscall 即 blocker）；POSIX 不给 dlopen 用户态取消出口（信号处理器内 longjmp 出 loader 会持锁离开 → 后续 `dl*` 死锁，属 UB）。**判例在案**：`:1100` A5 旧口径「按字面实现必然 fail、不可实现」→ 处置是**收窄判据文本**、非改实现；本例同构。
>
> **三处改动逐字前后对照（均为行内替换、零新增行）**：
>
> **① `:1029`**——改前原文「**唯一豁免 = `pthread_join`**：按第二趟修订集群 A5(b) 裁定 join 不可有界——仅限终态标志置位后调用，其阻塞无用户态超时出口，由观测窗到点与 host finally 收口兜底，`join-blocked-observed` 由 runner 依轮询超时登记，见 D-W 节「有界 join 可行性裁定」」；改后原文「**用户态无超时出口的调用恰两处、均为登记豁免**：**(1) `pthread_join`**——按第二趟修订集群 A5(b) 裁定 join 不可有界，仅限终态标志置位后调用，其阻塞无用户态超时出口，由观测窗到点与 host finally 收口兜底，`join-blocked-observed` 由 runner 依轮询超时登记，见 D-W 节「有界 join 可行性裁定」；**(2) P1 `dlopen`/`dlsym`**——同属无用户态超时出口的调用（A1 禁第二线程、`:355-360` 闭表无 alarm/timer/sigaction，且 loader 持锁期间无安全中止路径），其表内 10 s 为**预注册测量与分类参数、非中止阈值**（预算面见观测窗推导、位点分类面见 `APPFREEZE` 位点约束），出口按 D1 节「时间盒/超时分类」条 = 错误返回 ∨ 进程死亡 ∨ 观测窗到点，**探针侧不设中止分支，freeze 静态审查不得因此判不通过**」（该行其余文字逐字不变）。
>
> **② `:1034`**——改前原文「| P1 dlopen+dlsym | 10 s | 错误返回=observed-false 事实；未返回→死后收口路径（死亡事实记录（r9：原「死因分类」随死因表删除更名），见「死亡事实记录（证据向量）」节；现行 protocol 取值为 `complete`/`pre-only`，`post-mortem` 仅存于历史登记） |」；改后原文「| P1 dlopen+dlsym | 10 s（**测量与分类参数，非中止阈值**；`d1_elapsed_ms` 逐字落盘） | **盒到期无探针侧动作**；错误返回=observed-false 事实；未返回→死后收口路径（死亡事实记录（r9：原「死因分类」随死因表删除更名），见「死亡事实记录（证据向量）」节；现行 protocol 取值为 `complete`/`pre-only`，`post-mortem` 仅存于历史登记）；**调用返回但耗时超盒 → 仅逐字记录、不改 verdict**（沿 criteria-gap 判别方法 (1)） |」——时间盒列「10 s」→「10 s（**测量与分类参数，非中止阈值**；`d1_elapsed_ms` 逐字落盘）」；超时分类列首加「**盒到期无探针侧动作**；」、列尾（末 `|` 前）追加「；**调用返回但耗时超盒 → 仅逐字记录、不改 verdict**（沿 criteria-gap 判别方法 (1)）」；中间「未返回→死后收口路径（…）」逐字不变。
>
...
>
> **重审结论（2026-09-11，CC-4 生效手续）**：CC-4 经**跨厂商隔离重审**（重审席与本次 gate 3 复审席不同厂商）判**通过**——**0 blocker / 0 major / 2 minor**。重审席明确表态「CC-4 是正当的判据与证据格式同步——方向是**增加义务**：`:1034` 的「逐字落盘」一字未删（`:1034` 与 `05b2b53` 逐字 IDENTICAL）、`:521` 冻结扩展 payload、`:526` 写入落盘清单、实现侧真的落盘；若走「删掉该句」路线则 CC-3 只剩豁免、失去唯一能机器证明 P1 实际耗时的字段」，并**拒绝**「删掉 `:1034` 逐字落盘句」的替代路线。**本块「生效前提」已满足**，CC-4 自本日起生效，文首状态行转 `criteria-change-4-reviewed-pass-2026-09-11`。
>
> **重审 minor 处置**：
>
...
>
> **零新增行约束的声明与验证**：(a) 正文三处（`:268`/`:269`/`:1436`）+ 状态行 `:3` 一处，共 4 处均为**行内替换**（无换行插入、无行删除），`wc -l` = 改动前 **1862** → 四处替换后 **1862**（完全不变）→ 追加本登记块后 **1884**；锚点抽验 `sed -n '3p;268p;269p;521p;526p;530p;1029p;1034p;1059p;1067p;1079p;1162p;1231p;1232p;1434p;1436p'` 与父提交 `dc06640558b6190a1e29eb62b6ee85b6a6d431b4`（CC-5 前）同法逐行比对：仅 `:3`/`:268`/`:269`/`:1436` 为 CHANGED（均系本块所列的授权改动），其余 IDENTICAL。(b) `N1BDISC_*` 字面集合不变：`grep -o 'N1BDISC_[A-Z0-9_]*' | sort -u | sha256sum` = 改动前后同为 `44999c3eaaaef3edbc266d8659e365a1dc07a793df1bdd239baf7979351e684c`。(c) `git diff --unified=0` 正文均为**单行替换**（状态行 `@@ -3 +3 @@`、`:268`/`:269` 相邻两行 `@@ -268,2 +268,2 @@`、`:1436` `@@ -1436 +1436 @@`），唯一成块新增 = 本登记块。(d) `npx --no-install markdownlint-cli2 docs/n1b-disc-gate-plan.md` 告警数 **236 → 236**（无新增）。(e) `git diff --check` 无空白错误。
>
> **重审结论（2026-09-11，CC-5 生效手续）**：CC-5 经**跨厂商隔离重审**（重审席与实施席不同厂商）判**通过**——**0 blocker / 0 major / 2 minor**。重审席独立确认要点：`:268` 新冻结字面**逐字节等于**仓外 pair3 blocked record 的 `measured.software_version_trimmed`（35 字节）、**不等于** verbatim（36 字节，多 1 个尾随空格，属 `param get` 平台产物）；`SP6C` 是前冻结值的真实历史字面（两份 2026-09-06 记录逐字印证），非抄录讹误；第三对退役陈述在「判据登记块 / 仓外 JSON / 仓内终态节」三处交叉一致；其余冻结面（syscall 闭表、marker 集、时间盒表、MR 表、HDC 白名单、faultlogger 候选集、13 门）零改动、未新增 fail/blocked 面；CC-1~CC-4 登记块未被改写。**本块「生效前提」已满足**，CC-5 自本日起生效，文首状态行转 `criteria-change-5-reviewed-pass-2026-09-11`；仍须新 AUTH / pair / evidence 三 ID 并从 **gate 1** 重走门序列。
>
> **重审 minor 处置**：
>
> **m-1（冻结字面口径钉死不足）**：`:268` 冻结字面 = `software_version_trimmed`（35 字节）；实测 verbatim 为 36 字节、含 1 个尾随 ASCII 空格，该空格属 `param get` 平台输出产物，**不进入冻结元组**（沿 CC-2 冻结值口径与第二对 confirmation 的 `MATCH (逐字)` 判定口径）。
>
> **m-2（验证句基线写错）**：本块「零新增行约束的声明与验证」处原写「与 `git show HEAD:` 同法逐行比对」——提交后对 `HEAD:` 自比对恒 IDENTICAL，真实基线应为**父提交** `dc06640558b6190a1e29eb62b6ee85b6a6d431b4`。**据此更正**：该处表述已改为「与父提交 `dc06640558b6190a1e29eb62b6ee85b6a6d431b4`（CC-5 前）同法逐行比对」（与 CC-4 m-1 同类处置一致）。
