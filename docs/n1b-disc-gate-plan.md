...
**分域声明（r16，sol B-02；r17 重写为四子域，三席收敛，第十七轮 B-01；r18 扩为五子域 (a)-(e)；r19 补迟到竞态窗收口）**：`dw_return_class` 的求值按下列五子域收口——
...
**迟到竞态窗收口（r19，主会话——sol 第十九轮中断前报告的格：JT=1 ∧ 标志未置 ∧ capture 已有 RETURN）**：worker 冻结序为 写快照→RETURN→EXIT→置标志——RETURN 入 capture 与标志置位之间有真实窗口。该格的收口：**capture 有 RETURN 即不入 (d)**（(d) 前件「R 缺以 capture 为准」失败——标志未置的机器门不能否决 capture 的既成事实）；
该格下 P12 读快照须等标志（标志未置 → 快照门禁读）→ **主线程有界等待标志置位，冻结局部上界 = 1000 ms**（r19 定稿值；沿 D-W 节冻结轮询原语 = 原子标志 + 10 ms `clock_nanosleep`、非忙等；依据：worker 冻结序中 EXIT 发射与标志置位之间是**同一控制流的相邻语句**——EXIT 已入 capture 则标志置位只差一次调度切换，毫秒量级；1000 ms = 100 倍裕量）。
**盒到期处理（总函数 cutoff，r19 定稿——sol 第十九轮终稿建议：不得无界等待未知事件）**：1000 ms 内标志置位 → 读快照走 13 类 → complete pass（第三格钉）；
**1000 ms 到期标志仍未置 → `unobservable(cause=flag-race-window-expired)`**（新具名 cause——「worker 在 EXIT 前某点被永久抢占/阻塞、标志永不置」的不可判形态：进程活（非死亡分量）、RETURN 在（非 (d)）、标志未置（非 (e)）——三态均不吸附，独立收口）
...
（合法竞态残余宁缺勿误——RETURN 在 capture ⟹ 快照五 raw 已写完（单次读先写完才发 marker）只是标志未 release；F8(2) 比对用 class 派生值、runner 侧从 capture RETURN 正常重建，一致 → 不命中）。
**上界理由与外层窗的关系（r19 定稿注）**：1000 ms 局部盒叠加于 P12 位点内、含于观测窗收尾裕量（58 s）中，不改变全序/预算；**禁止**把外层观测窗 525 s 当作本等待上界（sol 第十九轮：外层窗不是本格的冻结局部上界、abandoned worker 可永久活挂——本格收口由 1000 ms cutoff 承载，与 525 s 无关）。
...
五态 `call-boundary-incomplete` → 逐字透传 `unobservable(cause=call-boundary-incomplete)`（**r13 第三包，sol M-01：五态定义明言该态「既可能尚未调用，也可能已进入调用」，「destroy 已过」对它不成立——r12 把它与 `call-returned` 同记 post-destroy 是事实错误，改沿 `dw_watchdog_killed` r13 有序互斥表 ② 同款透传自身 cause**）。
    - **(步 0) complete∧join-timeout 收口编码 1 值**（r17 新增；**r18 注：分域 (e)「标志已置∧capture 无 RETURN」为 fail(F8(2)) 收口、不赋 class 值，不入本表——本表只收可赋值形态**）——`unobservable(cause=poll-never-returned)` → 本字段输出逐字等于该输入编码本身（worker 未返回、`_C`-resolve 两维前提均不成立，沿 skip/死亡收口同款理由；非 skip、进程活到 P12、`join-timeout` 已登记、`DW_RETURN` 缺、终态标志未置五前件见分域声明 (d)（r19 扩为五前件））。
...
| A11 | **worker poll 返回后 raw 单次读同源两写的源码层静态核对（r19 新增，sol M-04）**：在**源码层**核对——worker 于 `poll` 返回后对 `ret`/`errno`/`revents` 各恰好一次读取、单调钟全局恰一次读取（`at_mono_ms` 与 `elapsed_ms` 为该单次钟读的两个导出值，无第二次钟/errno 读取点），共享快照六字段与 `DW_RETURN` marker 字段均自该次读取的同一组局部变量写出（同源两写——raw 一致性来自实现约束，F8(2) 只比派生值、不检测 raw 分叉）；任一不满足即 freeze 前 fail（反例 = 源码中任一二次读钟/读 errno 路径，或 marker 字段与快照字段非同源的控制流） |
...
| 3 freeze + 静态审查 | 判据 freeze；reviewer 静态审查（符号清单誊录核对、**实现配置字面 == 本文 MR 表字面（不一致即不得 freeze）**、静态断言 A1-A11（r10 纳入 A9、r18 纳入 A10，A M-04/B M-01）、时间盒表核对；**r4 第二趟 S1 新增：`Fault_Type` 候选集收敛前置——若届时已能取得目标元组的一条真实 faultlogger 文本，须据此把候选集收敛为实测字面并重新提交独立审查；若取不到，则以「死亡事实记录（证据向量）」节冻结候选集执行并在证据中登记该未实测依赖（r9：原「以死因分类节 S1 兜底出口执行」及其行 4 (iii)/行 5 分岔随死因表删除——冻结前候选集收敛义务本身保留，域外字面按该节证据规则 5 记 `other:<原值逐字>`、不驱动 fail）**） |
...
期望——`dw_return_class=unobservable(cause=poll-never-returned)`（r17 新值、19 值域内；非 skip、进程活到 P12、join-timeout 已登记、RETURN 缺、终态标志未置——五前件齐（r19；第五前件在本夹具恒真——worker 挂死未达置位序））、`dw_join_result=join-timeout`（**既有本体值——join 域不扩、10 值不变（r17 核对结论：该 cause 仅入 class 域）**）、`dw_destroy_distinguishable_from_timeout`=步 (0) 透传（输出逐字等于输入编码本身）；
