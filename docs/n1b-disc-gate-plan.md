...
**信号规格（r21 冻结——grok M-01）**：快照写入完成信号标识符 `dw_snapshot_written`（进程内原子布尔、初值 false、置位后不自旋不重置）——可见性 = seq_cst store（worker 侧）+ seq_cst load（P12 侧每次读：盒入口一读定进盒、盒到期再读一读**复核 SIG 单调性（违反 → F8(3)）**）；终态标志同批规格化：标识符 `dw_worker_terminal`、同规格（seq_cst store/load、初值 false、置位后不重置——原 release/acquire 仅为 :646 序边描述、无标识符与内存序规格）。
