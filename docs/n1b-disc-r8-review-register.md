...
## 九之十七、第二十四轮终账（三席齐：sol 2B/0M/1m、deepseek 1B/2M、grok 2B/0M/3m）

**去重 2 blocker（三席收敛，全部核实成立）：**
1. **`:672` 族无例外全称（三席精确收敛，grok 范围最全）**：`:672`「全部派生字段……重建比对」/`:680`「独立重建并比对」/F8 索引 `:1118`/criteria-gap (2) `:1345`——**四处操作句全部无 cut-aware 例外**（(B) 段自证「:665 族是烧 T\* 的机制」却未回写）。T\* 双 verdict。**修法（grok）：同一钉三处落盘——四处全加例外指针 + cut-state 头句/(B) 首句从「class」改「dw_outcome 会被迟到完成分叉的字段」。**
2. **允许集无机器闭表（三席收敛，grok 构造最深——T†）**：`:756`「如」=示例非闭表、flag-race 行未声明、「⑤ 或 unobservable」类型不闭（⑤ 本身是 unobservable）。**T†（grok 独有构造）**：诚实 flag-race∧EXIT=1∧F=0（EXIT 在 P12 决策**前**已发射——不是「迟到」、「迟到不参与」措辞救不了）→ 实现者按 watchdog ①-⑤ 唯一机器闭表见 EXIT→④ vs POST ⑤ → F8 fail。**修法：class × watchdog/poll raw 允许集闭表（至少 poll-never/flag-race 两行）+「EXIT 存在性不参与」写成 cut=false 一律（含决策前已发射）+ T† 钉。**
...
### deepseek 终稿——pass 0B/3M（全为派发稿勘误级，无文档缺陷）

**12 格全部通过**——含本轮最重要格（格 4 pre-only 迟到完成）：④ 语义句「waiter 正常走到终态并退出」与「被杀的线程不可能发出 EXIT 后再死」对该格**均为真**（worker 走完全序发 EXIT 后进程死——死亡分量正交登记、无矛盾）；格 8 RACEWIN 绑定「写值/发射点」二分自洽（A12 措辞覆盖发射判定的控制流位置）；格 12 主会话对 P2 残留②的裁决链路验证正确。3M 全为派发稿框定/行号/拼法计数勘误（M-01 指出 JT=0 pre-only 实际落 ④ 非 ⑤——机制正确；M-02 F=0⑤ 实际在 :1488；M-03 cut-imputed 实 7 处同拼零异拼）。块 2 计数/行宽/三处一致全过。

### grok 终稿——pass 0B/2M/3m（两次中断后完成，明确标注未走查区）

**第二十六轮终账：三席全 pass——sol 0B/0M/0m（首次三零票）、deepseek 0B/3M（勘误级）、grok 0B/2M/3m。去重 0 blocker。**

grok 走查的九个高风险点全部单值确认（T\*/T†/poll-never/迟到返回/C_wd/③ 可达性/FLAG 代理/合取/程序核对）；C_wd 闭合、主会话 F=1 钉裁决验证正确、pre-only 死亡格（本轮最高值构造）判定不升 B（carve-out 字面在 pre-only 不命中——cut 未载）。其 2M/3m 建议**在冻结说明里登记不阻塞**：M-01（适用域三合取与 carve-out 无优先序——pre-only 无 POST、cut 未载，缺省读 false 的最坏实现可推进 carve-out——修法一句「cut 仅 complete/POST 有定义、缺省不得当 false」）；M-02（:773「读不到 EXIT marker→写 ⑤」观测推导主句未删净——只留 cut-imputed 即可）；m-01/m-02/m-03（残留误引/全称过宽/cause 字面误导——均已登记）。

**「跨厂商隔离独立审查 0 blocker」正式达成**（r25 版 @ b12e671，三席隔离采样、绑定声明一致、验证链完整）。轨迹最终：12→8→4→3→5→3→2→1→2→3→5→1→2→1→**0**。

## 冻结记录（2026-09-02）

N1BDISC 判据（docs/n1b-disc-gate-plan.md，git 04cf222，1784 行）经用户显式授权冻结。达成路径：26 轮审查-修复循环（r8 起三席跨厂商隔离独立审查制），blocker 轨迹 12→8→4→3→5→3→2→1→2→3→5→1→2→1→0，最终票型 sol 0B/0M/0M / deepseek 0B/3M / grok 0B/2M/3m。冻结前 grok 两项 M 落地（适用域优先序/cut 缺省句、观测推导删净）；m-01/02/03 冻结说明登记项在案。

冻结后约束：判据修改 = 判据变更须重审；ID 分配（AUTH/pair、evidence）与一切物理执行以用户逐项显式授权为前置。

## CC-1 判据变更记录（2026-09-05）

冻结后首次判据变更 **CC-1**（实现阶段独立审查 B-04 blocker 触发，冻结记录上文不改）：

- **B-04 互斥（变更因由）**：原 `:368`（冻结编号，下同）「`dw_inwait_proc_fd` 两个 `/proc` 路径各占 `inst=1`/`inst=2`」与 `:831` in-wait 采集「≤2 s 窗口、10 ms 间隔重采样」在冻结 syscall 闭表 `:351-358`（无 `lseek`/`pread`，同 fd 二次 read 必返 0，每次采样必须重开文件）下互斥不可同时满足。
- **用户授权方案①**：用户（直接人类决策者）于 2026-09-05 会话内显式授权——`inst` 按创建时刻自 1 递增、不设两实例上限（三个候选修法中改动最小、保留全密度采样事实）。
- **重审（grok 席，2026-09-05，跨厂商隔离）**：**0 blocker / 2 major / 4 minor，通过**。M-1/M-2 已按重审建议在同一补丁内闭合；m-1（N-inst selftest 夹具）与 m-2（自陈防误引注）留待实现期随动，m-4（path↔inst 双射取消）由 A6/DW_INWAIT.src 承载，登记于案不阻塞。
