...
最后核验：2026-08-31（`ADJ-T0-N1B-20260831-0001` 扩展：N/G 门代码追认登记、发现型 campaign 记录形态）
...
- `<门>` 使用下列门代码之一；跨门记录使用产生该证据的最早门，并在“关联阶段/门”字段补充其他门：
  - 正式路线门 `R0` 至 `R10`；
  - Emulator 投入门 `E0` 至 `E8`；
  - native 路线门 `N0` 至 `N7`；其中 N1 与 N2 的拆分子门写作 `N1A`/`N1B`/`N2A`/`N2B`（证据 ID 中门代码使用大写无连字符形态，如 `EV-N1A-...`）；
  - 前置探针门 `G0`；
  - 发现型 campaign 门 `N1BDISC`（记录形态见下文「发现型 campaign 记录形态」）。

  `N0`/`N1A`/`G0` 已有既成证据 ID（如 `EV-N0-EMU24-20260810-0002`、`EV-N1A-EMU24-20260831-0008`、`EV-G0PHYS1API26-20260830-0001`），本次扩展是对既成先例的**追认登记**，不改写任何历史证据、既有判定或已消耗的 ID。
...
发现型 campaign（当前唯一实例：门代码 `N1BDISC`）只采集平台事实、不做功能判定。其记录沿用本 schema 的证据 ID 格式、必填字段与 `record_status` 枚举，`verdict` 枚举**不新增任何取值**，仍然只有 `pass | fail | blocked | invalid`；`collected` 只是 `record_status` 的取值，**严禁**写作 `verdict: collected`。

发现型 campaign 的 `verdict` 只对基础设施与完整性求值，不对平台行为求值：

| `verdict` | 含义 |
| --- | --- |
| `pass` | 发现协议按冻结顺序跑完、各项事实已登记落盘；事实本身可以是负面的（某平台能力不存在、调用被拒、无数据可采等），均不影响 `pass` |
| `fail` | 探针自身在基础设施正常的前提下未完成预注册采集，或破坏冻结顺序、未增量落盘、封签失败 |
| `blocked` | 仅限外部基础设施不可达（设备连接失败、HDC 退化、目标元组漂移、构建输入漂移） |
| `invalid` | 证据污染、哈希不一致、白名单外命令、跨 attempt 拼接 |

- 平台行为「不如预期」永远不是 `fail`，也不是 `blocked`，全部落入观察字段。
- 全部平台事实必须以三态之一逐项登记：`observed-true` / `observed-false` / `unobservable`。
- `record_status` 沿用现有语义：执行后 `collected`，独立审查合格后 `reviewed-pass`。
- 硬约束：发现型 campaign 的 `verdict: pass` **不得在任何后续文档中被引用为平台行为结论或任何门的功能结论**；它只表示「采集完整性成立」。其事实登记只能作为后续门判据的**设计输入**，不构成 fd 合同、数据面、E4 或任何门的 pass。
- 增量落盘要求：每项事实完成即发射 marker 并落盘，任一后续步骤崩溃不得损失已获事实。
