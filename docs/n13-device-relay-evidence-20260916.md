...
## relay 方向结题（2026-09-17，人类确认收口）

> 本节为事后追加，不改写既有正文。结题依据为人类决策；本节全部事实均照抄本文既有各节，不新增结论。

### 1. 结题依据（人类决策）

人类于 2026-09-17 本会话（DSH Web GUI）回复**「同意你的建议」**，确认按主会话建议**收口结题**（选项 A）。

### 2. 本方向已确立的结论（逐条，均为本文既有内容，不得加码）

1. **设备侧 relay 会话与 WG over relay 数据面成立**（D 轮：`wgSessions 0→2`、`vpn-tun RX 0→4624B`＝`wgRxToTun`，与运维对端 `net-host` 的会话及其侧 `created new wgProxy` + 0.08s 首次握手；见「中继/对端侧证据并入」节第 1 条与既有正文 §4/§5）。
2. **`18153fa`（signal `relayServerAddress`）适用范围**：对**官方客户端**＝其愿意向我方 OpenConn/建 lane 的**必要条件**（依据＝运维对端侧时间线 + D 轮实证）；对**我方实现的对端**＝**非必要**（L 类臂 A1 反证；见「L 类臂 A1 的反证与实验设计教训」节 §4）。
3. **对端掉线→重连后永久拒收缺陷**：根因（`docs/relay-presence-refresh-analysis-20260916.md`，带 file:line）→ 复现（先红）→ 修复 `0cfb723` → **离线忠实验证**（复现套件 5/5、全量 32 套件 476/0、3 连跑一致、`flake-check 30` → 0/30、`build.sh` exit 0；见「A2 拒收的根因闭环」节 §2–§5）。
4. **该修复的真机受控验证未取得支持**（V 类判定「未收敛/不完整」）——**不是被否证**，而是**观测能力不足**（见「V 类」节 §4/§7）。

### 3. 三处观测盲区（结题的直接原因，照抄 V 节判断）

1. 状态 JSON **不暴露「会话属于哪个 peer」**（`wgSessions=N` 无法归因到具体对端）；
2. **探针送达不可观测**（对端 `write failed=0` 只说明写成功；设备侧 `wgRxToTun=0` 不能区分「未送达」与「送达未解密」）；
3. **PWO 是否到达设备无逐行日志**——而该缺陷的触发条件恰是「presence 被显式置 false」。

### 4. 待证判断（不得写成结论）

V2（修复前）未复现缺陷 → 触发条件在现网可能不出现，该缺陷可能是**潜伏**而非现网活跃故障；根因之一可能是**自建中继的 presence 语义与上游（一次性 `PeersOnline` / 常驻 PWO）不一致**——**该中继语义从未核实**。

### 5. 未来若需真机级证明，先补三项诊断（记录为待办，非本次范围）

1. 状态 JSON 暴露 **per-peer 会话归属**（peer id / overlay IP ↔ 会话）；
2. 探针增加**回执计数**（sent / acked）；
3. **presence 转换打点**（`PeersOnline` / `PeersWithdrawnOffline` 逐行日志，带 peer id）。

### 6. 遗留的未证项汇总（逐条，均为本文既有内容）

1. `wgTxBytes=0`（出向业务流量未测，既有正文 §7-4）；
2. 3 peer 中第三个无会话（未追，既有正文 §7-3）；
3. `≈92B response` 逐帧判别未做（既有正文 §7-1）；
4. 多 peer 交错离线组合压测未覆盖（「A2 拒收的根因闭环」节 §8）；
5. N13 §3.3 第 3 条为**会话级**满足（中继不记录数据面，「投递」层面独立证据不存在；「中继/对端侧证据并入」节 §5）。

### 7. 口径不变

本文档全部结论仅 **N13 级证据**；**不构成 N2-H pass**；**不构成 N6 pass**；ICE/STUN 与 WG peer 端点未纳入冻结。

### 8. AUTH 生命周期（以仓B 目录 `ls` 实存为准）

- 本文实 `ls` 仓B `~/harmonyos-signing/netbird-n1bdisc/diagnostics/`：编号连续的生效件（`AUTH-DIAG-DEVICE-VALIDATION-<日期>-<NNNN>.json` + `.sha256` sidecar）与同名证据目录实存 **0001–0009** 共 9 个编号，其中 0001（20260913）早于本方向。本方向对应 **0002–0009**：0002（派发稿标注为越界事件登记；生效件 `AUTH-DIAG-DEVICE-VALIDATION-20260914-0002.json` 与同名目录实存）＋ 0003／0004／0005／0006／0007／0008／0009（各生效件与诊断目录均实存）。派发稿口径「7 份」与其括号所列 8 个编号不一致，**按 `ls` 实存如实登记为 0002–0009 共 8 个编号**。
- 全部已收尾：各生效件含 `consumed` 回填与 `purpose_completion_events`（各 json `issuance.amendment_events` 在案；本文各节配额收尾记录同）。
- **待办**：0005–0009 的人类**回读确认**——各生效件 `human_confirmation` 注记 `readback_required=true，回读用户确认待人类执行`（本文 grep 实读）；0003/0004 亦存 `readback_required` 字段，其人类回读是否已完成未逐份核验（待确认）。
