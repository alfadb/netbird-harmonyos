...
- [N1BDISC 第四对 pair 三 ID 分配授权登记（2026-09-11 · 0001，host-only，CC-5 重绑后重走门序列）](evidence/n1bdisc-authorization-2026-09-11-0001.md)
  ...
  - **下一步**：gate 4（设备侧，**须用户本人 `tconn`** + **恰一次**内存级 `list targets`）；gate 5 三探针对照 CC-5 重绑元组 `PLA-AL10 7.0.0.105(SP10C00E105R7P3)`；gate 13 Live 须用户全新确认（决议 §4.3.9）；执行前须关闭设备自动更新（首对/第三对两次 gate 5 漂移均系 OTA 所致）
