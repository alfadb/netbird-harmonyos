...
- [N1BDISC 发现 campaign 判据（r26，已冻结）](n1b-disc-gate-plan.md)
  ...
  - 判据变更（criteria change，均 `reviewed-pass`）：**CC-1**（2026-09-05）、**CC-2**（2026-09-06 元组重绑至 `PLA-AL10 7.0.0.105(SP6C00E105R7P3)`）、**CC-3**（2026-09-11 P1 `dlopen`/`dlsym` 时间盒语义收窄）、**CC-4**（2026-09-11 D1 `elapsed_ms` 落盘依据 + 467/525 上界法理修正）；第三对 pair 判据基线 = `04cf222` + CC-1~CC-4
...
- [N1BDISC 第四对 pair 三 ID 分配授权登记（2026-09-11 · 0001，host-only，CC-5 重绑后重走门序列）](evidence/n1bdisc-authorization-2026-09-11-0001.md)
  - `AUTH-N1BDISC-PHYS1API26-20260911-0001` / campaign `N1BDISC-PHYS1API26-20260911-0001` / evidence `EV-N1BDISC-PHYS1API26-20260911-0001`：用户 2026-09-11 会话内显式授权（选项 A）「再次 rebind 到新元组并以新三 ID 重走门序列」，触发事实 = 第三对 gate 5 元组漂移退役；三 ID 保持 **candidate 态未消费**（`consumed=false`/`reusable=false`、`is_evidence=false`），**任何门均未执行**
  - 判据基线 = `04cf222` + CC-1~CC-4（全 `reviewed-pass`）+ **CC-5（`criteria-change-5-pending-review-2026-09-11`；重审通过后方可按新元组恢复可 freeze/测量状态）**；新冻结元组 `PLA-AL10 7.0.0.105(SP10C00E105R7P3)`（CC-5 重绑）；前三对终态不复用、不继承、不撤销；`freeze-3-v4`、实现 `53faa1f`、CC-1~CC-4、pair-3 audit-1 为可复用资产；AGC bundle/Profile/PHYS-1 沿用无需重建
  - **下一步**：gate 1-3（host-only）；随后 gate 4-5 为设备侧，**须用户本人连接设备**（gate 5 三探针对照 CC-5 重绑元组）；gate 13 Live 须用户全新确认（决议 §4.3.9）；执行前须关闭设备自动更新（首对/第三对两次 gate 5 漂移均系 OTA 所致）
