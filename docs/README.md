...
- [E3-PHYS-PREFLIGHT 物理设备执行授权登记（2026-08-16 · 0003，当前）](evidence/e3-physical-preflight-authorization-2026-08-16-0003.md)
  - `AUTH-E3-PHYS1API26-20260816-0003`：新 pair `E3-PHYS-PREFLIGHT-20260816-0003` / `EV-E3-PHYS1API26-20260816-0003`，attempt initial / retry N/A；当前 `blocked-awaiting-full-gates`、not ready，machine confirmation/review/freeze/DryRun/Live 均 pending；gate 1 只允许 git/文件/进程表读取且绝不可调用 HDC，gate 4 才允许各一次 `tconn` 与 `list targets`；HAP source basis `62409c5f...` 与最终 runner/governance `code_sha` 独立冻结；0002 误触后由 daemon 以 host process termination 清理，进程表确认 HDC0，但命令级清理审计未持久化，故不计作合规 gate 操作、不声称 device evidence 或具体清理命令；E8 `CLOSED`
