...
- [N1BDISC 第三对 pair 三 ID 分配与候选本地提交授权登记（2026-09-06 · 0002，host-only，已退役）](evidence/n1bdisc-authorization-2026-09-06-0002.md)
  ...
  - 候选实现 `spikes/n1b-disc-phys-hap/` 全套 commit `ea44b87` 已快进合并入 `main` 并推送；本轮整改链 `53faa1f`/`4b81f53`/`5a56ba3`（+ 更早 `e53bb3d`/`dccd66e`）亦已推送；逐门登记见 [第三对授权登记「门序列执行登记」与「终态处置」](evidence/n1bdisc-authorization-2026-09-06-0002.md)；gate 4-5 已执行（2026-09-11，漂移退役），**设备侧元组需用户重新决策**；gate 13 Live 须用户全新确认（决议 §4.3.9）
