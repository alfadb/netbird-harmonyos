...
- [E3-PHYS-PREFLIGHT 物理设备 live 预检证据（API26 20260829-0001，reviewed-pass/pass，预检完结）](evidence/e3-physical-preflight-api26-20260829-0001.md)
  - `EV-E3-PHYS1API26-20260829-0001`：`execution: live`、`record_status: reviewed-pass`、`verdict: pass`；**S1-S7 全部 pass——项目首次完整 pass**（S2 Allow/onCreate/create/fd=33、S3 严格进程边界 terminal、S4 deny 全窗口无 B create、S5 应用信息强停 terminal、S6 B 显式冲突拒绝冻结码 `2203002`、S7 收尾 terminal）；**S7 前置校准实证生效**（B 残留 `:vpn` 进程 observed-only 记录不门控、A 正常门控）；执行窗口 2026-08-30 10:14-10:31 实测；cleanup `verified-clean`、integrity 空、166 次白名单 HDC；范围仅冻结元组 E3 可达性，不等于 E4-E7/产品/数据面结论，E8 保持 `CLOSED`
- [E3-PHYS-PREFLIGHT 物理设备执行授权登记（2026-08-29 · 0001，consumed-pass，预检完结）](evidence/e3-physical-preflight-authorization-2026-08-29-0001.md)
  - `AUTH-E3-PHYS1API26-20260829-0001`：完整执行 13 门，gate 13 Live 于 2026-08-30 S1-S7 全 pass（证据 `reviewed-pass/pass`）；**`consumed-pass`**、pair 已消费、**无后继 AUTH**——E3-PHYS-PREFLIGHT 预检就此完结，后续路线由用户另行治理；仓外对象逐字节保留不得复用
- [E3-PHYS-PREFLIGHT 物理设备 live 预检证据（API26 20260828-0001，reviewed-pass/blocked）](evidence/e3-physical-preflight-api26-20260828-0001.md)
  - `EV-E3-PHYS1API26-20260828-0001`：`execution: live`、`record_status: reviewed-pass`、`verdict: blocked`；**S1-S6 首次全 pass**（S2 机器验证 Allow/onCreate/create/fd、S3 严格进程边界 terminal、S4 deny 全窗口无 B create、S5 应用信息强停 terminal、S6 B 显式冲突拒绝冻结码 `2203002`）；S7 因 B 的 `:vpn` 进程在冲突拒绝后存活（平台行为，双处观察）被前置门 blocked；cleanup `verified-clean`、integrity 空、145 次白名单 HDC；reviewed-pass 指证据审查完成，不是 E3 pass
- [E3-PHYS-PREFLIGHT 物理设备执行授权登记（2026-08-28 · 0001，consumed-blocked）](evidence/e3-physical-preflight-authorization-2026-08-28-0001.md)
  - gates 1-12 pass；gate 13 Live 完成：S1-S6 pass / S7 blocked（前置模型与平台行为不符），证据 `reviewed-pass/blocked`；pair 已消费不得复用；superseded_by AUTH-E3-PHYS1API26-20260829-0001
...
- [E3-PHYS-PREFLIGHT 物理设备执行授权登记（2026-08-17 · 0001，governance-operation-invalid-retired）](evidence/e3-physical-preflight-authorization-2026-08-17-0001.md)
  - gate 1-8 pass；gate 9 agent 未经授权执行设备进程枚举，audit-2 文件存在但不构成有效 gate pass；gate 10-13 not-run，未 DryRun/Live、未 consumed，仓外对象字节保留且 pair 永久不可复用
...
- [N1BDISC 发现 campaign 判据（r26，已冻结）](n1b-disc-gate-plan.md)
  ...
  - 判据变更（criteria change，均 `reviewed-pass`）：**CC-1**（2026-09-05）、**CC-2**（2026-09-06 元组重绑至 `PLA-AL10 7.0.0.105(SP6C00E105R7P3)`）、**CC-3**（2026-09-11 P1 `dlopen`/`dlsym` 时间盒语义收窄）、**CC-4**（2026-09-11 D1 `elapsed_ms` 落盘依据 + 467/525 上界法理修正）、**CC-5**（2026-09-11 设备元组重绑至 `PLA-AL10 7.0.0.105(SP10C00E105R7P3)`，第三对 gate 5 漂移退役触发；经跨厂商隔离重审通过，0 blocker / 0 major / 2 minor 已处置）；第三对 pair 判据基线 = `04cf222` + CC-1~CC-4；第四对 pair 判据基线 = `04cf222` + CC-1~CC-5
...
- [N1BDISC 第三对 pair 三 ID 分配与候选本地提交授权登记（2026-09-06 · 0002，host-only，已退役）](evidence/n1bdisc-authorization-2026-09-06-0002.md)
  - `AUTH-N1BDISC-PHYS1API26-20260906-0002` / campaign `N1BDISC-PHYS1API26-20260906-0002` / evidence `EV-N1BDISC-PHYS1API26-20260906-0002`：三 ID 未消费（`consumed=false`/`reusable=false`、`is_evidence=false`）；**gate 1-3 pass（host-only）、freeze-3-v4 成立（0 blocker / 0 major / 2 minor）**；**终态（2026-09-11）：gate 5 元组漂移退役**——gate 4 恰一次内存级 `list targets`（`targets_count=1`，target 未输出未持久化）+ gate 5 三探针实测 `PLA-AL10 7.0.0.105(SP10C00E105R7P3)`，构建分支标签与冻结值 `SP6C00E105R7P3` 不同（数字版本 `7.0.0.105` 相同）→ `blocked-tuple-drift`；`retired-terminal`、`consumed: false`（未测量/未 Live）、`reusable: false`、**无后继 AUTH**；仓外 blocked record `records/target-binding-confirmation-pair3-20260911.json`（sha256 `aa664446723139c388176d026c8c0be64927f695d6b4c3abb767d475c7fbba8e`）；前两对 pair 终态不复用、不继承、不撤销
  - 候选实现 `spikes/n1b-disc-phys-hap/` 全套 commit `ea44b87` 已快进合并入 `main` 并推送；本轮整改链 `53faa1f`/`4b81f53`/`5a56ba3`（+ 更早 `e53bb3d`/`dccd66e`）亦已推送；逐门登记见 [第三对授权登记「门序列执行登记」与「终态处置」](evidence/n1bdisc-authorization-2026-09-06-0002.md)；gate 4-5 已执行（2026-09-11，漂移退役），**设备侧元组已由用户授权 rebind（CC-5 重绑至 `PLA-AL10 7.0.0.105(SP10C00E105R7P3)`）并以第四对新三 ID 重走门序列**；gate 13 Live 须用户全新确认（决议 §4.3.9）
- [N1BDISC 第四对 pair 三 ID 分配授权登记（2026-09-11 · 0001，host-only，CC-5 重绑后重走门序列）](evidence/n1bdisc-authorization-2026-09-11-0001.md)
  - `AUTH-N1BDISC-PHYS1API26-20260911-0001` / campaign `N1BDISC-PHYS1API26-20260911-0001` / evidence `EV-N1BDISC-PHYS1API26-20260911-0001`：用户 2026-09-11 会话内显式授权（选项 A）「再次 rebind 到新元组并以新三 ID 重走门序列」，触发事实 = 第三对 gate 5 元组漂移退役；三 ID 保持 **candidate 态未消费**（`consumed=false`/`reusable=false`、`is_evidence=false`），**gate 1-5 pass（gate 1-3 host-only + gate 4-5 设备侧；gate 5 元组逐字 MATCH）、freeze-4 成立（0 blocker / 0 major / 1 minor，含沿革勘误）**（逐门登记见[第四对授权登记「门序列执行登记」](evidence/n1bdisc-authorization-2026-09-11-0001.md)）
  - 判据基线 = `04cf222` + CC-1~CC-5（全 `reviewed-pass`；CC-5 = 设备元组重绑至 `PLA-AL10 7.0.0.105(SP10C00E105R7P3)`）；新冻结元组 `PLA-AL10 7.0.0.105(SP10C00E105R7P3)`（CC-5 重绑）；前三对终态不复用、不继承、不撤销；`freeze-3-v4`、实现 `53faa1f`、CC-1~CC-4、pair-3 audit-1 为可复用资产；AGC bundle/Profile/PHYS-1 沿用无需重建
  - **下一步**：gates 6-12（host-only，HDC0 已恢复；gate 8 用既有 AGC profile 重新签名 HAP）；gate 13 Live 须用户全新确认（决议 §4.3.9）
...
- **归档清单快照与联调脚本版本调和；判据加固重评零翻转（2026-09-12 晚，只追加）**：`archive-manifest-20260912.md`（sha256 `5912e328fc08e029c41149dc2bbcd65d3641fc9bab130c154595aec686d939e9`）为 2026-09-12 **不可变快照**，保持原样不改写；其「固定引用登记」登记的 `run-dns-joint-test.sh` sha256 `ba20dde13094cc39dd4754d9899079273e7c81c95d26f7eb7d3a76f1f42f84d8`（830 行）对应加固前版本，P0-4 判据加固后当前脚本为 863 行、sha256 `c11e452977a77b1cd0ce15adca4de63fb3659169684aa2b0d5b58c259dc055bb`（加固前内容备份 `run-dns-joint-test.sh.bak-20260912`，sha256 与清单登记值一致），版本调和记录 `~/harmonyos-signing/netbird-n1bdisc/records/artifact-version-note-20260912.md`（sha256 `39c8e5985984f7de8274506fc959df344506fe01231cb4c3e3d44f63a80538ac`，含 `.sha256` sidecar）；判据加固审查报告 `~/harmonyos-signing/netbird-n1bdisc/reviews/criterion-hardening-20260912.md`（sha256 `d757012aa09d244d2278e115c73b76acdc3ea1b5a9af394db44a0a6a0c945820`）；关键事实：**用加固后判据重评 N/R/P 三轮真实日志判据翻转 = 0，N 轮 `20260912T191913` 仍 PASS；假阳性探针 A/B 由旧 PASS 变为新 FAIL**。该判定仍是 ad-hoc 脚本判定：`is_evidence:false`，不是门（gate）结论，不构成任何新授权，不产生任何门级 verdict。
- **N1BDISC campaign 终态关闭（2026-09-12 追加）**：campaign 登记终态关闭（`closed-scope-achieved-transferred-to-implementation`，仓B `~/harmonyos-signing/netbird-n1bdisc/records/campaign-terminal-n1bdisc-20260912.json`，sha256 `73ec96d4a3fa1785b0560f78611b21a4a936e89cb000247aacd962aa6bcef618`，sidecar OK）——pair4 的 freeze-4 不再约束工作区，**实施阶段 = N3-N6 作为实现里程碑 + 每门一次受治理验证**；§三（N6 未 pass 前不得开启产品实现）与 §四（N3 书面法律前置）不放宽，新测量须新授权新三 ID。登记见 [n1b-disc-handoff-20260902.md](n1b-disc-handoff-20260902.md)「N1BDISC campaign 终态关闭（2026-09-12 追加）」子节。
- **诊断授权类别落地（2026-09-13，只追加）**：[evidence-schema.md](evidence-schema.md)「E8 前唯一物理设备例外」段之后已增补第二个显式例外「例外二（2026-09-13 追加）」（诊断授权类别：不消费 campaign/pair/evidence ID、不产生门结论的开发性设备操作；≤单会话或 24 h、session-scoped 四字段消费语义、人类逐条确认签发、agent 不得自分配 AUTH ID）；AUTH 模板见 `~/harmonyos-signing/netbird-n1bdisc/diagnostics/AUTH-TEMPLATE-diagnostic-20260913.json`（含 `.sha256` sidecar）；登记见 [native-nx-governance.md](native-nx-governance.md)「追加登记：诊断授权类别（2026-09-13，只追加不改写）」节。
- **许可路线改为 AGPL（2026-09-13，只追加）**：用户 2026-09-13 决定本项目整体由 MIT 改为 AGPL-3.0 发布（历史版本按当时声明授权，不回溯）；「N3 书面法律前置 + 净室纪律」整条路线据此作废，T0 缩范围提案所附 C1-C6 条件不再落地；唯一现行条款见 [native-nx-governance.md](native-nx-governance.md) 文末「§四 修订（2026-09-13，唯一现行）：许可路线改为 AGPL」节；仓B 登记 `~/harmonyos-signing/netbird-n1bdisc/records/license-route-decision-20260913.json`（含 `.sha256` sidecar）。
