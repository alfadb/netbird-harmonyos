...
- [N1BDISC 第三对 pair 三 ID 分配与候选本地提交授权登记（2026-09-06 · 0002，host-only）](evidence/n1bdisc-authorization-2026-09-06-0002.md)
  - `AUTH-N1BDISC-PHYS1API26-20260906-0002` / campaign `N1BDISC-PHYS1API26-20260906-0002` / evidence `EV-N1BDISC-PHYS1API26-20260906-0002`：三 ID 保持候选态未消费（`consumed=false`/`reusable=false`、`is_evidence=false`）；**gate 1-3 pass（host-only）、freeze-3-v4 成立（0 blocker / 0 major / 2 minor）**，下一步 **gate 4**（设备侧，须用户连接设备）；前两对 pair 终态不复用、不继承、不撤销
  - 候选实现 `spikes/n1b-disc-phys-hap/` 全套 commit `ea44b87` 已快进合并入 `main` 并推送；本轮整改链 `53faa1f`/`4b81f53`/`5a56ba3`（+ 更早 `e53bb3d`/`dccd66e`）亦已推送；逐门登记见 [第三对授权登记「门序列执行登记」](evidence/n1bdisc-authorization-2026-09-06-0002.md)；gate 4-5 需用户连接设备，gate 13 Live 须用户全新确认（决议 §4.3.9）
