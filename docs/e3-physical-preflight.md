...
| Settings re-allow | 预测路径冻结为 `direct-system-activation`；路径偏差只作预注册观测，不因偏差本身 blocked；无 Settings 入口或无法重新激活仍 blocked。`ADJ-20260808-0002`/`0003` 后（强模式）：S5 撤销机制为 `settings-app-info-force-stop` 机器化原子步骤——fresh A Start/create accepted/open → 机器校验 Settings>应用信息>A（`settings-app-info` layout gate：Settings owner + A 标签 + Force Stop 控件）→ 机械强制停止 → `<bundle>:vpn` 连续 absent + bundle present；Settings>VPN 页不再询问（`settings_vpn_page_capture=not-required`，绝不作为 invalid/block/pass 输入）；HDC 只 `PidOf`/`BundleDump` 观察，绝不 ForceStop（HDC force-stop cleanup-only）。**历史** `ADJ-20260807-0003` 的 Settings>VPN observation + 人工 `SETTINGS-APP-INFO-FORCE-STOP-CAPTURED` 确认仅 historical，不得作为新 campaign 规则 |
...
- 不加入 `protect`、外部 endpoint、数据泵、后台服务、TestRunner 或自动授权。系统授权、Settings 撤销（`ADJ-20260808-0002`/`0003` 后为机器化 Settings 应用信息强制停止：fresh A → 机器校验 Settings A app-info → 强制停止 → `:vpn` probes）和冲突操作必须由普通用户可见 UI 完成并由机器 layout gate 校验截图；**历史** `ADJ-20260807-0003` 的人工 Settings>应用信息强制停止 + 截图确认与 Settings>VPN observation 仅 historical（新 campaign 不再询问）。
...
5. **Settings revoke**（`ADJ-20260808-0002`/`0003` 强模式）：预测路径为 `direct-system-activation`，路径偏差只记录为预注册观测（`machine-layout-and-event-classified`），不因偏差本身 blocked；无 Settings 入口或无法重新激活仍 blocked。撤销机制为 `settings-app-info-force-stop` 机器化原子步骤：先 fresh A Start/create accepted/open（clean reactivation 基础，机器 layout gate 校验 entry/authorization、绑定 request、`CREATE_ACCEPTED` 后 process-target verify）；随后操作员仅做两个机械步骤——打开 A 应用信息页（机器校验 `settings-app-info` layout：Settings owner + A 应用标签 + Force Stop 控件；无匹配 → invalid）、执行强制停止（机器 capture/layout gate + `:vpn` 连续 absent + bundle present）；HDC 只 `PidOf`/`BundleDump` 观察，绝不 ForceStop（HDC force-stop 明确 cleanup-only，仅 finally 残留清理，Reason 限 `exception-cleanup`/`final-cleanup`）。`ADJ-20260808-0003` 后 S5 **不再要求** Settings>VPN 页 observation 步骤，也不再索取人工 `SETTINGS-APP-INFO-FORCE-STOP-CAPTURED` 确认（`settings_vpn_page_capture=not-required`，绝不作为 invalid/block/pass 输入）。pass 需机器校验、fresh create/open、bundle still present、`<bundle>:vpn` Extension 进程连续 absent（≥2 次、间隔 ≥3s；`ADJ-20260808-0001` 后 `PidOf` 只探测 `<bundle>:vpn`，探针记录含 `process_target`；调度实际达到冻结 3.0s——精确 DateTimeOffset round-trip + 0.1s 裕量，间隔不足时继续探而不是提前 Finished）；无 `UI_STOP` 正常；缺机器校验、缺 fresh create、bundle absent 或 Extension 进程未连续 absent 均 `blocked`；app-info layout 无法机器确认（Settings app-info 真实结构尚未采样）→ invalid（下一 Live fail-closed 风险）。**历史** `ADJ-20260807-0003` 的 Settings>VPN observation + 人工确认仅 historical，不得作为新 campaign 规则。
6. **第二 VPN 冲突**（`ADJ-20260808-0002` 机器判定）：唯一 A request `CREATE_ACCEPTED`；唯一 B request `CREATE_REJECTED` 且明确冻结冲突码 `2203002`；事件窗口无双 accepted、无意外 Start/Stop/错误次序。**删除** no_dual/dual 作为判据（字段可 null/non-authoritative）。任何额外操作 → scenario invalid 并立即停止后续。`ADJ-20260808-0003` 细化：A create rejected/invalid fd 为**功能 fail**（非操作员 invalid，不再请求 B、S7 `not-run-after-functional-fail`，finally cleanup 仍执行）；B 拒绝码为非冻结码 → **blocked**（平台结果，非 invalid）；B 被拒后 B Extension 进程可能仍存活，仅 observed、绝不要求 absent；A 侧窗口末精确 `pidof <bundle>:vpn` present 校验保持（`Get-ExactProcessCheckpoint`）。**历史** operator 三态 `NO-DUAL-ACTIVE-CAPTURED`/`DUAL-ACTIVE-CAPTURED` 仅 historical，不再是当前规则。若需引用旧文，见 historical 段落。旧三态摘要（historical only）：仅 `dual_active_confirmed=true` 且 `no_dual_active_confirmed=false` 为 `fail`（`dual-active-observed`）；`noDual=true` 且 `dual=false` 为正常；双 false 为 `blocked`（`dual-active-observation-unresolved`）；双 true 为 `blocked`（`inconsistent-operator-confirmation`）；留空/false 单独绝不直接 fail。历史 record 曾含 `no_dual_active_confirmed`/`dual_active_confirmed`/`operator_state`（`ADJ-20260808-0002`/`0003` 后 deprecated null/non-authoritative）。
...
scenarios:
  ...
  scenario_5_settings_revoke:
    ...
    app_info_machine_gate: pass | invalid # machine settings-app-info layout gate (Settings owner + exact A label + Force Stop/强制停止 control; NO un-sampled app-info-structure id/key requirement — ADJ-20260808-0003); unverified => invalid (App Info real structure not yet sampled => next Live fail-closed risk). NO ExpectedBundle: A/B same-name cannot manufacture a false pass — A correctness is proven by the A process effect gate below (force-stop -> A <bundle>:vpn absent + bundle present), not by a bundle field on the Settings page
    app_info_force_stop_capture: <name/status/machine_verified> # runner field: scenario-5-app-info-force-stop capture + machine layout gate
    assertions:
