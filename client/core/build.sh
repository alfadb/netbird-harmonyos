#!/usr/bin/env bash
# NetBird client core — host-only cross build + verification (no device, no
# signing). Promoted from spikes/n1b-disc-phys-hap/probe/build.sh (frozen
# offline+locked chain on the 26.0.0.821 baseline); product flavor.
#
# Usage: bash client/core/build.sh
#
# Environment (frozen, mirrors the spike):
...
#   llvm-ar); linker/sysroot/-l args also pinned in .cargo/config.toml.
#
# Steps:
...
# --- 4. authoritative build: offline + locked ---
log "cargo build --release --target $TARGET --offline --locked"
...
# (dump to a file first: `nm | grep -q` under pipefail SIGPIPEs on early exit)
"$LLVM_BIN/llvm-nm" -D "$OUT" > "$ROOT/target/symbols.dyn" || die "llvm-nm failed"
MISSING=0
for sym in x25519_secret_key x25519_public_key x25519_key_to_base64 \
    ...
    if ! grep -q " T ${sym}$" "$ROOT/target/symbols.dyn"; then
        echo "MISSING: $sym" >&2
        MISSING=1
    fi
done
...
for sym in napi_module_register napi_create_function napi_get_cb_info \
    ...
    grep -q " U ${sym}$" "$ROOT/target/symbols.dyn" || die "expected imported symbol $sym not found"
done
if grep -qE ' U (napi_create_threadsafe_function|napi_create_async_work)$' "$ROOT/target/symbols.dyn"; then
    die "A1 violation: threadsafe-function / async-work import found"
fi
log "import surface OK (napi/hilog from host process)"

# NAPI module registration constructor must be wired into .init_array
"$LLVM_BIN/llvm-readelf" -r "$OUT" > "$ROOT/target/rel.dyn" || die "llvm-readelf -r failed"
grep -q 'R_AARCH64_RELATIVE' "$ROOT/target/rel.dyn" || die "no relative relocations (init_array constructor unwired?)"
