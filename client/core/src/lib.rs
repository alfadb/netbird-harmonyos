...
//! | `connector_status()` | `() => string` | connector snapshot -> `{running,state,started_at_unix,last_update_unix,peer_count,route_count,reconnects,last_error,session_expiry,session_expires_at_unix,session_renew_attempts,wg_apply_failed,wg_apply_errors,logout_ok,terminal,ice:{peers,idle,gathering,checking,connected,disconnected,failed,endpoints_applied,reachable,last_error},signal:{registered,reconnects,last_error},wg:{fed_socket,fed_tun,device_up,ready,peers_with_session,handshakes,tx_packets,rx_packets,dropped_no_route,decrypt_errors}}` (error = class + status code only, never key material; N3-7 `terminal` = worker ended by itself — fatal/exhausted — the shell tears the VPN down on it; N5c `ice` = per-peer ICE summary; N5d `signal` = real signal link state; N7 `wg` = REAL data-plane status of the device-backed seam) |
